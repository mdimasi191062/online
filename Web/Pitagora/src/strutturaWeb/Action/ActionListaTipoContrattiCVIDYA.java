...
public class ActionListaTipoContrattiCVIDYA implements ActionInterface
{
  public ActionListaTipoContrattiCVIDYA()
  {
  }
   public String esegui(HttpServletRequest request, HttpServletResponse response) throws Exception
  {
    ...
    for(int i=0;i<view.getVector().size();i++)
    {
      ...
      if(temp.getFLAG_SYS().compareToIgnoreCase("S")==0 && 
         (temp.getCODE_TIPO_CONTR().compareToIgnoreCase("13")==0 || 
          temp.getCODE_TIPO_CONTR().compareToIgnoreCase("1")==0 ||
          temp.getCODE_TIPO_CONTR().compareToIgnoreCase("42")==0 ||
          temp.getCODE_TIPO_CONTR().compareToIgnoreCase("44")==0 ||
          temp.getCODE_TIPO_CONTR().compareToIgnoreCase("49")==0
          )
        )
      {
        vect.add(temp);
