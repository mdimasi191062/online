...
import oracle.jdbc.OracleTypes;  
  ...
  public class ActionListaTipoContrattiRientriTI implements ActionInterface
  {
    public ActionListaTipoContrattiRientriTI()
    {
    }
     public String esegui(HttpServletRequest request, HttpServletResponse response) throws Exception
    {
      ...
      for(int i=0;i<view.getVector().size();i++)
      {
        ...
        if(temp.getFLAG_SYS().compareToIgnoreCase("S")==0 && 
           (temp.getCODE_TIPO_CONTR().compareToIgnoreCase("9")==0 || 
           temp.getCODE_TIPO_CONTR().compareToIgnoreCase("17")==0 ||
          temp.getCODE_TIPO_CONTR().compareToIgnoreCase("23")==0 ||
          temp.getCODE_TIPO_CONTR().compareToIgnoreCase("37")==0)
          )
        {
          vect.add(temp);
        }
