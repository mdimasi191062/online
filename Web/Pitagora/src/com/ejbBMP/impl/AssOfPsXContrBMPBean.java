...
import com.ejbBMP.AssOfPsXContrBMPPK;
import com.utl.*;
import java.util.*;
import java.rmi.*;
import java.sql.*;
...
public class AssOfPsXContrBMPBean extends AbstractEntityCommonBean implements EntityBean
{
  ...
  public AssOfPsXContrBMPPK ejbCreate(String codeContr, String dataIniOfPs, String codeOf,
                                      ...
                                      throws CreateException, RemoteException, CustomEJBException
  {
    pk=new AssOfPsXContrBMPPK();
     ...
     try
      ...
      OracleCallableStatement cs=(OracleCallableStatement)conn.prepareCall("{call " + StaticContext.PACKAGE_SPECIAL + ".ASSOC_OFPS_X_CONTR_INSERISCI(?,?,?,?,?,?,?,?,?,?,?,?,?)}");
      ...
      if ((cs.getInt(12)!=DBMessage.OK_RT)&&(cs.getInt(12)!=DBMessage.NOT_FOUND_RT))
        throw new EJBException("DB:ASSOC_OFPS_X_CONTR_INSERISCI:"+cs.getInt(12)+":"+cs.getString(13));
  ...
  public AssOfPsXContrBMPPK ejbFindByPrimaryKey(AssOfPsXContrBMPPK primaryKey) throws FinderException, CustomEJBException, RemoteException
  {
...
//  //System.out.println(">>> disattiva");
   pk = (AssOfPsXContrBMPPK) ctx.getPrimaryKey();
    ...
    try
      ...
      cs.execute();
       ...
       if (cs.getInt(7)!=DBMessage.OK_RT)
         throw new Exception("DB:ASSOC_OFPS_X_CONTR_DISATTIVA:"+cs.getInt(7)+":"+cs.getString(8));
      cs.close();
      // chiusura connessione
      conn.close();
    }
...
//    //System.out.println(">>> ejbRemove");
    ...
    try
    {
      conn = getConnection(dsName);
       ...
       OracleCallableStatement cs=(OracleCallableStatement)conn.prepareCall("{call " + StaticContext.PACKAGE_SPECIAL + ".ASSOC_OFPS_X_CONTR_CANCELLA(?,?,?,?,?,?,?)}");
