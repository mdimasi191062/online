...
public class CmbFunzioniSTLBean extends AbstractSessionCommonBean implements SessionBean 
{
  public Vector getFunzioni() throws RemoteException, CustomException
  {
   ...
   try
      ...
      OracleCallableStatement cs=(OracleCallableStatement)conn.prepareCall("{call " + StaticContext.PACKAGE_SPECIAL + ".FUNZION_LISTA(?,?,?)}");

      // Impostazione types I/O
      
        
      cs.registerOutParameter(1,OracleTypes.ARRAY, "ARR_FUNZION");
      ...
      for (int i=0;i<dati.length;i++)
          {
              ...
              if (attr[0] != null)
                  elem.setCodeFunzione(attr[0].stringValue());
              else
                  elem.setCodeFunzione("");
              if (attr[1] != null)
                  elem.setDescFunzione(attr[1].stringValue());
              else    
                  elem.setDescFunzione("");
  ...
  public Vector getFunzioniUpload() throws RemoteException, CustomException
  {
   ...
   try
      ...
      OracleCallableStatement cs=(OracleCallableStatement)conn.prepareCall("{call " + StaticContext.PACKAGE_SPECIAL_ONLINE + ".FUNZION_LISTA_UPLOAD(?,?,?)}");
