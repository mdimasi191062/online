...
<%=StaticMessages.getMessage(3006,"cbn1_vis_scarti_penali_cl.jsp")%>
</logtag:logData>
<% 
  ...
  response.addHeader("Cache-Control", "no-store");
    ...
    //Stringa contente l'eventuale errore  
  String error    = null;
  String bgcolor="";
  ...
  strCode_account=request.getParameter("Code_account");
...
<SCRIPT LANGUAGE="JavaScript" SRC="../../common/js/size.js"></SCRIPT>
...
<%
    appoVector = remote_I5_2RECORD_SCARTATO_CL.findAll_CPM(Code_account,flag_sys,Code_elab);
...
<form name="frmSearch" onsubmit="submitFrmSearch('0');return false" method="post" action="cbn1_vis_scarti_penali_cl.jsp">
...
%>
                        <TR>
                           <TD bgcolor='<%=bgcolor%>' class='text'><%=aRemote[j].getDesc_Motivo_Scarto()%></TD>
                           <TD bgcolor='<%=bgcolor%>' class='text'><%=aRemote[j].getCode_Istanza_Ps()%></TD>
                           <TD bgcolor='<%=bgcolor%>' class='text'><%=aRemote[j].getTipo_Scarto()%></TD>                           
                        </tr>
