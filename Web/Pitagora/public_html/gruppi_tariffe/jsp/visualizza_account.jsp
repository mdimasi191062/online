...
<%=StaticMessages.getMessage(3006,"visualizza_account.jsp")%>
</logtag:logData>
...
//GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  {
    accountVector = remoteEnt_TipoRelazioni.getAccount(strAccount,Gruppo);
...
function submitRicerca(typeLoad)
{
  ...
  objForm.action = "visualizza_account.jsp";
  objForm.submit();
}
...
function SubmitSeleziona()
{
  ...
  {
    opener.frmDati.strCodeAccount.value = objForm.CodAccountSel.value;
    opener.frmDati.strCodeGruppo.value = '<%=Gruppo%>';
    self.close();
    if ( opener.frmDati.strCodeGruppo.value == -1 )
    {
      opener.frmDati.strDescAccount.value = objForm.DescAccountSel.value;
      opener.frmDati.strInizioValid.value = objForm.DIVAccountSel.value;
    }
    else {
                          ...
                          <tr>
                            <td bgcolor="<%=StaticContext.bgColorCellaBianca%>" width="2%">
                                ...
                                if (i==0)
                                {
                                  ...
                                  %>
                              <input type='radio' checked name='SelAccount' value='<%=myelement.getDESC_ACCOUNT()%>' onclick=ChangeSel("<%=myelement.getCODE_ACCOUNT()%>","<%=myelement.getDATA_INIZIO_VALID()%>",'<%=i%>')>
                                  <%
                                }
                                else
                                {
                                  %>
                              <input bgcolor='<%=StaticContext.bgColorCellaBianca%>'  type='radio' name='SelAccount' value='<%=myelement.getDESC_ACCOUNT()%>' onclick=ChangeSel('<%=myelement.getCODE_ACCOUNT()%>',"<%=myelement.getDATA_INIZIO_VALID()%>",'<%=i%>')>
                                  <%
                                }
                                %>
                            </td>
    ...
    <td>
	    <table width="100%" border="0" cellspacing="0" cellpadding="0" align='center'>
          ...
          <td class="textB" bgcolor="<%=StaticContext.bgColorTestataTabella%>" align="center" colspan="5">
            ...
            <input type=hidden name="DIVAccountSel" value="<%=div%>">
	        </td>
	      </tr>
	    </table>
    </td>
