...
<%=StaticMessages.getMessage(3006,"cbn1_ins_tar_x_tipol_contr_acquisti_2_cl.jsp")%>
</logtag:logData>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function Initialize()
	{
		...
		setObjProp(objForm.txtContributoFriaco,"label=Contributo|obbligatorio=si|tipocontrollo=importo|maxnumericvalue=999999999999,999999|maschera=<%=StaticContext.nfInputMask%>"); 
		...
		setObjProp(objForm.txtAccessoCanoneFriaco,"label=Accesso|obbligatorio=si|tipocontrollo=importo|maxnumericvalue=999999999999,999999|maschera=<%=StaticContext.nfInputMask%>");
    ...
    // Fabio del 30/04/2004 inizio
		Enable(objForm.CHIUDI);
    // Fabio del 30/04/2004 fine

		Enable(objForm.txtDescrizioneTariffa);
		//Enable(objForm.txtDataInizioValidita);

		if((strCodeOggFatt == "<%=StaticContext.OF_CONTRIBUTO_INSTALLAZIONE%>")||(strCodeOggFatt == "<%=StaticContext.OF_CONTRIBUTO_INSTALLAZIONE_AMPL%>") || (strCodeOggFatt == "<%=StaticContext.OF_CONTRIBUTO_INSTALLAZIONE_TRASM%>")){
		...
		if((strCodeOggFatt == "<%=StaticContext.OF_CANONE_MENSILE_ITC%>")||(strCodeOggFatt == "<%=StaticContext.OF_CANONE_MENSILE_AMPL_CANALI%>")){
			Disable(objForm.txtUnitaMisura);
			DisableRadio(objForm.rdoTipoImporto,true);
  ...
  // Fabio del 30/04/2004 fine
	...
	function ONCONFERMA()
	{
		if(validazioneCampi(objForm)){
			...
			objForm.action = "<%=StaticContext.PH_TARIFFE_JSP%>cbn1_ins_tar_x_tipol_contr_acquisti_3_cl.jsp";
	...
	<tr>
		<td>
		 <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
			<tr>
			  <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%"><%=strTitoloPagina%> TARIFFA PER TIPOLOGIA DI CONTRATTO ACQUISTI</td>
