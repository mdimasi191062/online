...
<%=StaticMessages.getMessage(3006,"cbn1_ver_ndc_2_cl.jsp")%>
</logtag:logData>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function Initialize()
	{
        ...
        {%>
          ...
          strURL+="?messaggio=Per i seguenti account<BR>la relativa nota di credito non � stata congelata";
          strURL+="&strCodiceTipoContratto=<%=codiceTipoContratto%>";
		  strURL+="&strCodeFunz=<%=StaticContext.RIBES_INFR_BATCH_NOTE_CREDITO%>";
	...
	function preparePageFields()
	{
		...
		if(objForm.rdoAccountElaborati.length==null){
			strAppo = getRadioButtonValue(objForm.rdoAccountElaborati).split("|")[0];
		}else{
	...
	function ONCONGELA_TUTTI(){
		if(window.confirm("<%=remoteEnt_AnagraficaMessaggi.getAnagraficaMessaggi(StaticContext.LIST,"3112")%>")){
			//var arrAccountSelezionato = getRadioButtonValue(objForm.rdoAccountElaborati).split("|");
			//var strCodeAccount = arrAccountSelezionato[0];

			var strURL="<%=StaticContext.PH_NOTEDICREDITO_JSP%>cbn1_ver_ndc_3_cl.jsp";
	...
	function ONCONGELA_SEL(){
		if(window.confirm("<%=remoteEnt_AnagraficaMessaggi.getAnagraficaMessaggi(StaticContext.LIST,"3112")%>")){
			...
			//objForm.hidViewState.value = strCodeAccount;
	...
	function ONVISUALIZZA_SCARTI(){
		...
		strURL+="&hidCodeFunz=<%=StaticContext.RIBES_INFR_BATCH_NOTE_CREDITO%>";
	...
	<tr>
		<td>
		 <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
			<tr>
			  <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">Verifica batch note di credito: <%=request.getParameter("hidDescTipoContratto")%></td>
		...
		<td width='100%' colspan="3">
			<table >
				<tr>
	                     ...
	                     <td  class="text">
	                       <select class="text" name="cboNumRecXPag" onchange="reloadPage('1','cbn1_ver_ndc_2_cl.jsp');">
					  ...
					  <%   for(i=((pagerPageNumber.intValue()-1)*intRecXPag);((i < intRecTotali) && (i < pagerPageNumber.intValue()*intRecXPag));i++)	
					       {
								...
								<tr bgcolor="<%=bgcolor%>">
									<td width="4%" height="20" class="text">
										<input  <%=strChecked%> type="radio" name="rdoAccountElaborati" onClick="click_rdoAccountElaborati();" value="<%=lobj_AccountElaborati.getCODE_ACCOUNT()%>$<%=lobj_AccountElaborati.getCODE_PARAM()%>|<%=lobj_AccountElaborati.getTIPO_FLAG_ERR_BLOCC()%>|<%=lobj_AccountElaborati.getVALO_NR_SCARTI_NB()%>|<%=lobj_AccountElaborati.getDESC_ACCOUNT()%>|<%=i%>">
									</td>
