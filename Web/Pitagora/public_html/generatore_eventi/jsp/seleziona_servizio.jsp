...
<%=StaticMessages.getMessage(3006,"seleziona_servizio.jsp")%>
</logtag:logData>
...
//GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  if (typeLoad!=0)
    servizioVector = (Vector) session.getAttribute("servizioVector");
  else
  {
    servizioVector = remoteEnt_Inventari.getServizi();
    if (servizioVector!=null)
      session.setAttribute("servizioVector", servizioVector);
  }
//FINE GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//memorizza il numero di record totali
int intRecTotali=servizioVector.size();
...
function ChangeSel(codiceservizio,indice)
{
  objForm.CodServizioSel.value=codiceservizio;
  objForm.DescServizioSel.value=eval('objForm.SelServizi[indice].value');
}

function submitRicerca(typeLoad)
{
  var stringa="";
  stringa=objForm.txtServizio.value;
  objForm.txtServizio.value=stringa.toUpperCase();
  objForm.hidTypeLoad.value = "0";
  objForm.action = "seleziona_servizio.jsp";
  objForm.submit();
}
...
function SubmitSeleziona()
{
  ...
  {
    opener.frmDati.srcCodeServizio.value = objForm.CodServizioSel.value;
    opener.frmDati.strDescServizio.value = objForm.DescServizioSel.value;
    opener.frmDati.srcCodeAccount.value = '';
    opener.frmDati.strDescAccount.value = '';
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                <tr>
                    <td>
                        <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                              <tr>
                                <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Seleziona il servizio</td>
                            ...
                            <tr>
                                <td>
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                                        ...
                                        <tr>
	                                           <td width="40%" class="textB" align="right">Descrizione Servizio:&nbsp;</td>
	                                           <td width="40%" class="text"><input class="text" type='text' name='txtServizio' value='<%=strServizio%>' size='15'></td>
...
%>
                                    <tr>
                                        ...
                                        <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Descrizione Servizio</td>
                                        <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Codice Servizio </td>
                                    </tr>
	...
	<%for(j=((pagerPageNumber.intValue()-1)*intRecXPag);((j < intRecTotali) && (j < pagerPageNumber.intValue()*intRecXPag));j++)	
      ...
      {
        DB_Servizio myelement=new DB_Servizio();
        myelement=(DB_Servizio)servizioVector.elementAt(j);
        if ((i%2)==0)
...
<%
          if (i==0)
          {
            codiceservizio=myelement.getCODE_SERVIZIO();
            descservizio=myelement.getDESC_SERVIZIO();
%>
                
                                            <input type='radio' checked name='SelServizi' value='<%=myelement.getDESC_SERVIZIO()%>' onclick=ChangeSel("<%=myelement.getCODE_SERVIZIO()%>",'<%=i%>')>
<%
          }
          else
          {
%>
                                            ...
                                            <input bgcolor='<%=StaticContext.bgColorCellaBianca%>'  type='radio' name='SelServizi' value='<%=myelement.getDESC_SERVIZIO()%>' onclick=ChangeSel('<%=myelement.getCODE_SERVIZIO()%>','<%=i%>')>
<%
          }
%>
                                        </td>
	    ...
	    <table width="90%" border="0" cellspacing="0" cellpadding="0" align='center'>
	      <tr>
     		<td class="textB" bgcolor="<%=StaticContext.bgColorTestataTabella%>" align="center" colspan="5">
				...
				<input type=hidden name="CodServizioSel" value="<%=codiceservizio%>">
				<input type=hidden name="DescServizioSel" value="<%=descservizio%>">
	        </td>
	      </tr>
	    </table>
    </td>
