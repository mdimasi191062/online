...
<%=StaticMessages.getMessage(3006,"seleziona_causale.jsp")%>
</logtag:logData>
...
<%
  ...
  if (strTipo==null ) {
      strTipo= (String) session.getAttribute("parstrTipo");
  }else{
       session.setAttribute("parstrTipo",strTipo); 
  }
  ...
  String strCausale = Misc.nh(request.getParameter("txtCausale"));
...
//GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  if (typeLoad!=0)
    causaleVector = (Vector) session.getAttribute("causaleVector");
  else
  {
    causaleVector = remoteEnt_Inventari.getCausali();
    if (causaleVector!=null)
      session.setAttribute("causaleVector", causaleVector);
  }
//FINE GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//memorizza il numero di record totali
int intRecTotali=causaleVector.size();
...
function ChangeSel(codicecausale,indice)
{
  objForm.CodCausaleSel.value=codicecausale;
  objForm.DescCausaleSel.value=eval('objForm.SelServizi[indice].value');
}

function submitRicerca(typeLoad)
{
  var stringa="";
  stringa=objForm.txtCausale.value;
  objForm.txtCausale.value=stringa.toUpperCase();
  objForm.hidTypeLoad.value = "0";
  objForm.action = "seleziona_causale.jsp";
  objForm.submit();
}

function SubmitSeleziona(tipo)
{
  ...
  //  alert (objForm.CodCausaleSel.value);
  //  alert (objForm.DescCausaleSel.value);
 //   if ( opener.frmDati.cmdCausaleAtt.disabled == false  ) {
    if ( tipo == 'A'  ) {
        opener.frmDati.strTipoCausaleAtt.value = objForm.CodCausaleSel.value;
        opener.frmDati.strDescCausaleAtt.value = objForm.DescCausaleSel.value;
    } else {
        opener.frmDati.strTipoCausaleCes.value = objForm.CodCausaleSel.value;
        opener.frmDati.strDescCausaleCes.value = objForm.DescCausaleSel.value;
    }
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                <tr>
                    <td>
                        <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                              <tr>
                                <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Seleziona la causale</td>
                            ...
                            <tr>
                                <td>
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                                        ...
                                        <tr>
	                                           <td width="40%" class="textB" align="right">Descrizione Causale:&nbsp;</td>
	                                           <td width="40%" class="text"><input class="text" type='text' name='txtCausale' value='<%=strCausale%>' size='15'></td>
...
<%
    ...
    String desccausale="";
    String bgcolor="";
    if ((causaleVector==null)||(causaleVector.size()==0))
    {
%>
                                    <tr>
...
%>
                                    <tr>
                                        ...
                                        <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Descrizione Causale</td>
                                        <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Codice Causale </td>
                                    </tr>
	...
	<%for(j=((pagerPageNumber.intValue()-1)*intRecXPag);((j < intRecTotali) && (j < pagerPageNumber.intValue()*intRecXPag));j++)	
      
      {
        ...
        myelement=(DB_TipoCausaleNew)causaleVector.elementAt(j);
        if ((i%2)==0)
...
<%
          if (i==0)
          {
            codicecausale=myelement.getCODE_TIPO_CAUSALE();
            desccausale=myelement.getDESC_TIPO_CAUSALE();
%>
                
                                            <input type='radio' checked name='SelServizi' value='<%=myelement.getDESC_TIPO_CAUSALE()%>' onclick=ChangeSel("<%=myelement.getCODE_TIPO_CAUSALE()%>",'<%=i%>')>
<%
          }
          else
          {
%>
                                            ...
                                            <input bgcolor='<%=StaticContext.bgColorCellaBianca%>'  type='radio' name='SelServizi' value='<%=myelement.getDESC_TIPO_CAUSALE()%>' onclick=ChangeSel('<%=myelement.getCODE_TIPO_CAUSALE()%>','<%=i%>')>
<%
          }
%>
                                        </td>
	    ...
	    <table width="90%" border="0" cellspacing="0" cellpadding="0" align='center'>
	      <tr>
     		<td class="textB" bgcolor="<%=StaticContext.bgColorTestataTabella%>" align="center" colspan="5">
				<input type="button" class="textB" name="Seleziona" value="Seleziona" onClick="if (this.disabled) return false; else SubmitSeleziona('<%=strTipo%>');">
				<input type="button" class="textB" name="Annulla" value="Annulla" onClick = "javascript:CancelMe();">
				<input type=hidden name="CodCausaleSel" value="<%=codicecausale%>">
				<input type=hidden name="DescCausaleSel" value="<%=desccausale%>">
	        </td>
	      </tr>
	    </table>
    </td>
