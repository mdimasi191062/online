...
<%=StaticMessages.getMessage(3006,"seleziona_componente.jsp")%>
</logtag:logData>
...
<%
  ...
  String strComponente = Misc.nh(request.getParameter("txtComponente"));
...
//GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  if (typeLoad!=0)
    compoVector = (Vector) session.getAttribute("compoVector");
  else
  {
    compoVector = remoteEnt_Inventari.getComponenti(Code_Servizio,Code_Offerta,Code_Prodotto);
    if (compoVector!=null)
      session.setAttribute("compoVector", compoVector);
  }
//FINE GESTIONE CARICAMENTO VETTORE@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//memorizza il numero di record totali
int intRecTotali=compoVector.size();
...
<TITLE>Seleziona Componente</TITLE>
...
function ChangeSel(codicecomponente,indice)
{
  objForm.CodComponenteSel.value=codicecomponente;
  objForm.DescComponenteSel.value=eval('objForm.SelComponenti[indice].value');
}

function submitRicerca(typeLoad)
{
  var stringa="";
  var URLParam = '?Servizio=' + '<%=Code_Servizio%>' + '&Offerta=' + '<%=Code_Offerta%>'  + '&Prodotto=' + '<%=Code_Prodotto%>';
  stringa=objForm.txtComponente.value;
  objForm.txtComponente.value=stringa.toUpperCase();
  objForm.hidTypeLoad.value = '1';
  objForm.action = "seleziona_componente.jsp" + URLParam;
  objForm.submit();
}
...
function SubmitSeleziona()
{
  ...
  {
    opener.frmDati.strCodeComponente.value = objForm.CodComponenteSel.value;
    opener.frmDati.strDescComponente.value = objForm.DescComponenteSel.value;
    opener.CallParentWindowFunction(self);
  //  self.close();
  }
}
function CancelMe()
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                <tr>
                    <td>
                        <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                              <tr>
                                <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Seleziona la Componente</td>
                            ...
                            <tr>
                                <td>
                                    <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                                        ...
                                        <tr>
	                                           <td width="40%" class="textB" align="right">Descrizione Componente:&nbsp;</td>
	                                           <td width="40%" class="text"><input class="text" type='text' name='txtComponente' value='<%=strComponente%>' size='15'></td>
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorCellaBianca%>">
                          <tr>
                            ...
                            <tr>
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                                    <tr>
                                        <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Componenti Selezionate</td>
                                        <td bgcolor="<%=StaticContext.bgColorCellaBianca%>" class="white" align="center" width="9%"><img src="<%=StaticContext.PH_COMMON_IMAGES%>quad_blu.gif"></td>
                                    </tr>
                                </table>
                            </td>
...
<%
    ...
    String bgcolor="";
    if ((compoVector==null)||(compoVector.size()==0))
    {
%>
                                    <tr>
...
%>
                                    <tr>
                                        ...
                                        <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Descrizione Componente</td>
                                        <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Codice Componente </td>
                                    </tr>
	...
	<%for(j=((pagerPageNumber.intValue()-1)*intRecXPag);((j < intRecTotali) && (j < pagerPageNumber.intValue()*intRecXPag));j++)	
      
      {
        ...
        myelement=(DB_Componente)compoVector.elementAt(j);
        if ((i%2)==0)
...
<%
          if (i==0)
          {
            codicecomponente=myelement.getCODE_COMPONENTE();
            desccomponente=myelement.getDESC_COMPONENTE();
%>
                
                                            <input type='radio' checked name='SelComponenti' value='<%=myelement.getDESC_COMPONENTE()%>' onclick=ChangeSel("<%=myelement.getCODE_COMPONENTE()%>",'<%=i%>')>
<%
          }
          else
          {
%>
                                            ...
                                            <input bgcolor='<%=StaticContext.bgColorCellaBianca%>'  type='radio' name='SelComponenti' value='<%=myelement.getDESC_COMPONENTE()%>' onclick=ChangeSel('<%=myelement.getCODE_COMPONENTE()%>','<%=i%>')>
<%
          }
%>
                                        </td>
                                        <td bgcolor='<%=bgcolor%>' class='text'><%=myelement.getDESC_COMPONENTE()%></td>
                                        <td bgcolor='<%=bgcolor%>' class='text'><%=myelement.getCODE_COMPONENTE()%></td>
                                    </tr>
                                    <tr>
                                        <td colspan='3' bgcolor="<%=StaticContext.bgColorCellaBianca%>"><img src="<%=StaticContext.PH_COMMON_IMAGES%>pixel.gif" width="1" height='2'></td>
                                    </tr>
	    ...
	    <table width="90%" border="0" cellspacing="0" cellpadding="0" align='center'>
	      <tr>
     		<td class="textB" bgcolor="<%=StaticContext.bgColorTestataTabella%>" align="center" colspan="5">
				...
				<input type=hidden name="CodComponenteSel" value="<%=codicecomponente%>">
				<input type=hidden name="DescComponenteSel" value="<%=desccomponente%>">
	        </td>
	      </tr>
	    </table>
    </td>
