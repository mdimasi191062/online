...
function ONSELEZIONA()
{
  ...
  if(objForm.hidDescTipoContratto.value == 'CDN'){
    alert('Questa funzionalità è disponibile dal menù : \"Tariffe (Nuovo)\"');
    return;
  }
  <%}%>
  ...
  if(objForm.hidDescTipoContratto.value == 'CDN'){
    alert('Questa funzionalità è disponibile dal menù : \"Elab.Attive (Nuovo)\"');
    return;
  }
  <%}%>
...
<%  
    ...
    Vector lvct_TipiContratto = null;
    if ( intFunzionalita == 999 ) {
        lvct_TipiContratto = (Vector)remoteEnt_TipiContratto.getTipiContrattoFilter("50,51,60", "S");
    } else if ( intFunzionalita == 998 ) {
        lvct_TipiContratto = (Vector)remoteEnt_TipiContratto.getTipiContrattoFilter("54,55,56", "S");
    } else {
        lvct_TipiContratto = (Vector)remoteEnt_TipiContratto.getTipiContratto(strTCNonAmmessiClassic,strTCNonAmmessiSpecial);
    }
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>" align = "center">
					<tr>
						<td>
						 <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
							<tr>
							  <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">Classic</td>
    ...
    <%
	  ...
	  <%	intCount = 1;
           ...
           <% if(strAppoFlgForIav.equals(objDBTipoContratto.getFLAG_SYS()))
           		{
			if(intCount == 1){%>
				<tr>
          <td bgcolor='<%=StaticContext.bgColorHeader%>' class='white' colspan="2" >
          ...
          <tr>
            <td width='2%'>
				<%if (i==0){
				  ...
				  <input type='hidden' name='hidDescrizione<%=i%>' value='<%=objDBTipoContratto.getDESC_TIPO_CONTR()%>'>
