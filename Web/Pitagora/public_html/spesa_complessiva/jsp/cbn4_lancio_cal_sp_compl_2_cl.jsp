...
<%=StaticMessages.getMessage(3006,"cbn4_lancio_cal_sp_compl_2_cl.jsp")%>
</logtag:logData>
...
<%!
	...
	static final int gintStepClientiStatoProvvisorio3 = 4;
	...
	private Vector costruzioneVectorDataBean(Vector pvct_Account) throws Exception{
		...
		for (int j=0;j<pvct_Account.size();j++){
			...
			lobjAppoAccount.setCODE_GEST((String)lvctAppoDatiAccount.elementAt(0));
...
<%
	...
	String strMessage = "";
	String lstr_PeriodoRiferimento = "";
	...
	lstr_PeriodoRiferimento = Misc.nh(request.getParameter("cboPeriodoRif"));
	strDataIns = Misc.nh(request.getParameter("hidDataIns"));
//	Vector vctAppo = Misc.split(lstr_PeriodoRiferimento,"-");
	
	strMeseRif = strDataIns.substring(0,2);
	strAnnoRif = strDataIns.substring(2);
	...
	switch (intStepNow){
		case gintStepElabBatchXlancio:  
			  ...
			  strMessage ="Attenzione! per i seguenti clienti � in corso un'altra eleborazione batch!<br>";
		...
		case gintStepClientiStatoProvvisorio1:
			...
			strMessage ="Per i seguenti clienti esistono elaborazioni batch <br>";
		...
		case gintStepClientiStatoProvvisorio2:
	        ...
	        strMessage += " Proseguire escludendo gli account individuati?";																	
		break;
		case gintStepClientiStatoProvvisorio3:
			if (strtypeLoad.equals("1")) //paginazione
			  {
				lvct_AccountPagina = (Vector) session.getAttribute("lvct_AccountPagina");
			  }
        ...
        default:
			...
			<%  strParameterToLog = Misc.buildParameterToLog(lvct_AccountRisultato);
				String strLogMessage = "remoteCtr_SpesaComplessiva.lancioBatch(" + "CodeTipoContratto=" + lstr_CodeTipoContratto 
																				+ ";CodeUtente=" + strCodeUtente
																				+ ";MeseRif=" + strMeseRif
																				+ ";AnnoRif=" + strAnnoRif
																				+ ";DataFinePeriodo=" + strDataFinePeriodo
																				+ ";" + strParameterToLog  + ")";%>
			<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
					<%=StaticMessages.getMessage(3501,strLogMessage)%>
			</logtag:logData>
			<%strResult = remoteCtr_SpesaComplessiva.lancioBatch (lstr_CodeTipoContratto,
                            										   strCodeUtente,
                            										   strMeseRif,
											                           strAnnoRif,
											                           strDataFinePeriodo,
...
<HEAD>
	...
	<script language="JavaScript">
		...
		strURL += "?strImageDirPadre=spesa_complessiva";
		strURL += "&strViewType=GEST";
...
//			document.frmDati.action = "cbn4_lancio_cal_sp_compl_cl.jsp";
...
<form name="frmDati" action="cbn4_lancio_cal_sp_compl_2_cl.jsp" method="post">
...
for(i=0;i < lvct_AccountRisultato.size();i++){
	...
	if(strCodiceAccountRisultato.equals("")){
		strCodiceAccountRisultato = lobjAppoAccount.getCODE_GEST() + "$" + lobjAppoAccount.getCODE_PARAM();
	}else{
		strCodiceAccountRisultato += "|" + lobjAppoAccount.getCODE_GEST() + "$" + lobjAppoAccount.getCODE_PARAM();
	}
}%>
<input type = "hidden" name = "cboPeriodoRif" value="<%=lstr_PeriodoRiferimento%>">
...
<input type = "hidden" name = "txtDataFinePeriodo" value="<%=strDataFinePeriodo%>">
...
if(!blnExit){
	for(i = 0; i < lvct_AccountPagina.size(); i++){	
		...
		if(lstr_CodiciAccountDaEliminare.equals("")){
			lstr_CodiciAccountDaEliminare = lobj_Account.getCODE_GEST() + "$" + lobj_Account.getCODE_PARAM();
		}else{
			lstr_CodiciAccountDaEliminare += "|" + lobj_Account.getCODE_GEST() + "$" + lobj_Account.getCODE_PARAM();
		}
	}
