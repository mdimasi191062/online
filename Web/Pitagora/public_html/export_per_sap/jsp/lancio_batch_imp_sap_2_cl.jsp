...
<%=StaticMessages.getMessage(3006,"lancio_batch_imp_sap_2_cl.jsp")%>
</logtag:logData>
...
<EJB:useBean id="remoteCtr_Batch" type="com.ejbSTL.Ctr_Batch" scope="session">
  ...
  <%
  String strNameFileTestata = Misc.nh(request.getParameter("txtNameFileTestata"));
  String strNameFileDettaglio = Misc.nh(request.getParameter("txtNameFileDettaglio"));
  ...
  %>
	<% 	strLogMessage = "remoteCtr_Batch.LancioBatch_ImportPerSap:(strNameFileTestata:" + strNameFileTestata + ";strNameFileDettaglio=" + strNameFileDettaglio + ";CodeUtente=" + strCodeUtente + ";)";%>
	<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
			<%=StaticMessages.getMessage(3501,strLogMessage)%>
	</logtag:logData>
	<%
	lstr_Messagge = remoteCtr_Batch.LancioBatch_ImportPerSap(strNameFileTestata, strNameFileDettaglio, strCodeUtente);
...
<BODY onload="initialize()">
	<form name="frmDati" action="lancio_batch_imp_sap_2_cl.jsp" method="post">
	...
	<tr>
		<td>
		 <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
			<tr>
			  <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">Lancio Batch Import Sap</td>
