...
<%=StaticMessages.getMessage(3006,"lancio_batch_acq_dati_provs_2.jsp")%>
</logtag:logData>
...
<EJB:useBean id="remoteCtr_Batch" type="com.ejbSTL.Ctr_Batch" scope="session">
  ...
  <%
	...
	String strCodeBatch = Misc.nh(request.getParameter("hidCodeElabBatch"));
  ...
  %>
	<% 	strLogMessage = "remoteCtr_Batch.LancioBatch_Import_Provisioning:(CodeBatch:" + strCodeBatch + ";CodeUtente=" + strCodeUtente + ";)";%>
	<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
			<%=StaticMessages.getMessage(3501,strLogMessage)%>
	</logtag:logData>
	<%
	lstr_Messagge = remoteCtr_Batch.LancioBatch_Import_Provisioning(strCodeBatch, strCodeUtente);
