...
<%
  ...
  EURIBOR_CICLI_X_SERV_ROW riga = null;
  Float VALO_EURIBOR=null;
  String DATA_CONCAT="";
  String strDATA_INIZIO_CICLO = "";
  String strDATA_FINE_CICLO = ""; 
...
function Salva()
{
  ...
  if(parseFloat(window.document.frmDati.VALO_EURIBOR.value.replace(',','.'))==0){
...
<% 
  ...
  {    
      if (eur_j2.checkxInserimento()==0) // significa che non si può procedere con l'inserimento
      {
      ...
      else
      {
        ...
        riga = eur_j2.loadObjectInsert();
        DATA_CONCAT = riga.getPeriodo_rif();
        strDATA_INIZIO_CICLO = df.format(riga.getData_inizio_ciclo());
        strDATA_FINE_CICLO = df.format(riga.getData_fine_ciclo());        
      }
  }  
  else // caso modifica e cancellazione
  {  
        EURIBOR_CICLI_X_SERV_ROW[] aRemoteEURIBOR = (EURIBOR_CICLI_X_SERV_ROW[]) session.getAttribute("aRemoteEURIBOR");
        riga = aRemoteEURIBOR[Integer.parseInt(CodSel)];
        Float valore = riga.getValo_euribor();
        java.util.Date DATA_INIZIO_CICLO = riga.getData_inizio_ciclo();
        java.util.Date DATA_FINE_CICLO = riga.getData_fine_ciclo();   
        if(eur_j2.checkBatch()>0)
        {
        ...
        {
               if (eur_j2.checkCongelamento(valore,DATA_INIZIO_CICLO,DATA_FINE_CICLO)>0)
              {
        ...
        if (strMessaggioFunz.equals(""))
        {
                ...
                VALO_EURIBOR = valore;
                DATA_CONCAT = riga.getPeriodo_rif();
                strDATA_INIZIO_CICLO = df.format(DATA_INIZIO_CICLO);
                strDATA_FINE_CICLO = df.format(DATA_FINE_CICLO);                        
        }
  ...
  <tr>
    <td>
      ...
      <tr>
        ...
        <td  class="text" align="left"><%= strDATA_INIZIO_CICLO %>
        <input type="hidden" name="DATA_INIZIO_CICLO" value="<%=strDATA_INIZIO_CICLO%>"></TD>                                 
      </tr>           
      <tr>
        <td  class="textB" align="left">Data Fine Ciclo Fatt.:</td>              
        <td  class="text" align="left"><%= strDATA_FINE_CICLO %>
        <input type="hidden" name="DATA_FINE_CICLO" value="<%=strDATA_FINE_CICLO%>"></TD>            
      </tr>                   
