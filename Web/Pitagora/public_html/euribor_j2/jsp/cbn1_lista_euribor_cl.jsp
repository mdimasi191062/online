...
<%
  ...
  java.util.Date DATA_INIZIO_CICLO = null;
  java.util.Date DATA_FINE_CICLO = null;
  
  if(strDataDa!=null)
  {
    if (!strDataDa.equals(""))
      DATA_INIZIO_CICLO = df.parse(strDataDa);
  }
  ...
  {
    if (!strDataA.equals(""))
    DATA_FINE_CICLO = df.parse(strDataA); 
  }
...
// typeLoad=1 Variabile session
  ...
  {   
    appoVector = eur_j2.findAll(DATA_INIZIO_CICLO, DATA_FINE_CICLO);
    if (!(appoVector==null || appoVector.size()==0)) 
    {
      aRemoteEURIBOR = (EURIBOR_CICLI_X_SERV_ROW[])appoVector.toArray(new EURIBOR_CICLI_X_SERV_ROW[1]);
      session.setAttribute( "aRemoteEURIBOR", aRemoteEURIBOR);
    }  
    else
    {
      session.setAttribute("aRemoteEURIBOR", null);
    }  
  } 
...
<form name="frmDati" method="post" action='ListaEuribor.jsp'>
  ...
  <% 
  if ((aRemoteEURIBOR==null)||(aRemoteEURIBOR.length==0))  
                    ...
                    <tr>
                      <td>
                        ...
                        <tr>
                          ...
                          <td bgcolor='#D5DDF1' class="textB" width="20%">Data Inizio Ciclo</td>
                          <td bgcolor='#D5DDF1' class="textB" width="20%">Data Fine Ciclo</td>                         
                          <td bgcolor='#D5DDF1' class="textB" width="20%">Descrizione Servizio</td> 
                        </tr>
<pg:pager maxPageItems="<%=records_per_page%>" maxIndexPages="10" totalItemCount="<%=aRemoteEURIBOR.length%>">
...
<%
      ...
      for(j=((pagerPageNumber.intValue()-1)*records_per_page);((j<aRemoteEURIBOR.length) && (j<pagerPageNumber.intValue()*records_per_page));j++)
      {
         if ((i%2)==0)
          bgcolor="#EBF0F0";
         else
         ...
         String strDescrizioneServizio =  aRemoteEURIBOR[j].getServizio_rif();
         if (CodSel!=null)
         {
...
%>
                        <TR>
                           ...
                           <TD bgcolor='<%=bgcolor%>' class='text'><%= strDATA_INIZIO_CICLO %></TD>                                 
                           <TD bgcolor='<%=bgcolor%>' class='text'><%= strDATA_FINE_CICLO %></TD> 
                           <TD bgcolor='<%=bgcolor%>' class='text'><%= strDescrizioneServizio %></TD> 
