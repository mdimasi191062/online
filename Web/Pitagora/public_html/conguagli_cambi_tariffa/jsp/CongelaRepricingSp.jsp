...
<%=StaticMessages.getMessage(3006,"CongelaRepricingSp.jsp")%>
</logtag:logData>
...
function ChangeSel(codice,descAcc,erroreSel,nrgScarti,numRPC,param)
{
  ...
  document.accElabForm.numRepricingTrovate.value=numRPC;
...
function ONCONGELA_TUTTI()
{
 ...
 if (document.accElabForm.CONGELA_TUTTI.disabled) 
  ...
  else 
  {
   ...
   //new
      document.accElabForm.action = "CongelaRepricing2Sp.jsp";
...
function ONVISUALIZZA_SCARTI()
{
 if (document.accElabForm.VISUALIZZA_SCARTI.disabled) 
  ...
  else 
  {
     ...
     var verifica="verificaRepricing";
...
<form name="accElabForm" method="get" action="CongelaRepricingSp.jsp">
...
<%
   ...
   else
   {
       ...
       accElab = remoteLstAccElab.getLstAccRepricing(code_elab, cod_tipo_contr,flagTipoContr);
    ...
    if (accElab!=null && accElab.size()!=0)
    {
       ...
       numRepricingTrovate=String.valueOf(acc.getNumNC());
...
%>
                ...
                <pg:param name="errore" value="<%=errore%>"></pg:param>
                 <pg:param name="repricing" value="<%=repricing%>"></pg:param>
                 <pg:param name="numRepricingTrovate" value="<%=numRepricingTrovate%>"></pg:param>
                ...
                %>
                    ...
                    {
                      LstAccElabElem obj=(LstAccElabElem)objs[i];
                      //System.out.println(">>>>>>>>>>"+obj.getNumNC());
                      if(obj.getNumNC()==0)
                          repricing=repricing+"0";
                        else  repricing=repricing+"-";
                        if(obj.getFlagErrore().equalsIgnoreCase("S"))
                          errore="true";
                      
                    }
                    //System.out.println("repricing"+repricing);
                %>    
