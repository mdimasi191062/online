...
<%=StaticMessages.getMessage(3006,"cbn1_lancio_repricing_2_cl.jsp")%>
</logtag:logData>

<EJB:useHome id="homeCtr_Repricing" type="com.ejbSTL.Ctr_RepricingHome" location="Ctr_Repricing" />
<EJB:useBean id="remoteCtr_Repricing" type="com.ejbSTL.Ctr_Repricing" scope="session">
    <EJB:createBean instance="<%=homeCtr_Repricing.create()%>" />
</EJB:useBean>
	...
	switch (intStepNow){
        ...
        //paginazione
  			lvct_AccountPagina = (Vector) session.getAttribute("lvct_AccountPagina");
				...
				lvct_AccountPagina = remoteCtr_Batch.getElabBatchXLancio(lstr_CodeTipoContratto,
      ...
      //estrazione del code utente loggato dalla sessione
			...
			<%strParameterToLog = Misc.buildParameterToLog(lvct_AccountRisultato);
				strLogMessage = "remoteCtr_Repricing.lancioBatch(" + "CodeTipoContratto=" + lstr_CodeTipoContratto + ";CodeUtente=" + strCodeUtente + ";"  + strParameterToLog  + ")";%>
			<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
					<%=StaticMessages.getMessage(3501,strLogMessage)%>
			</logtag:logData>
			<%

			lstr_Messagge = remoteCtr_Repricing.lancioBatch (lstr_CodeTipoContratto,
                                                       strCodeUtente,
                                      								 lvct_AccountRisultato);
	...
	<script language="JavaScript">
		...
		function initialize()
		{
			...
			<%}else{
				...
				String strUrl = request.getContextPath() + "/classic_common/jsp/genericMsg_cl.jsp?message=" + java.net.URLEncoder.encode(lstr_Messagge,com.utl.StaticContext.ENCCharset); 
...
//			document.frmDati.action = "cbn1_lancio_repricing_cl.jsp";
...
<BODY onload="initialize()">
	<form name="frmDati" action="cbn1_lancio_repricing_2_cl.jsp" method="post">
