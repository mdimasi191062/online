...
<%=StaticMessages.getMessage(3006,"lancio_rep_swn_2_cl.jsp")%>
</logtag:logData>
...
<%	String strNameFile = Misc.nh(request.getParameter("txtNameFile"));
	...
	String strDataCreazione = Misc.nh(request.getParameter("txtDataCreazione"));
	...
	<% 	strLogMessage = "remoteCtr_Batch.LancioBatch_Consuntivo_Rep:(strNameFile:" + strNameFile + ";strPathFile=" + strPathFile + ";strDataCreazione=" + strDataCreazione + ";)";%>
	<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
			<%=StaticMessages.getMessage(3501,strLogMessage)%>
	</logtag:logData>
	<%
	lstr_Messagge = remoteCtr_Batch.LancioBatch_Consuntivo_Rep(strNameFile,
													           strPathFile,
                                                               strDataCreazione,
														       strCodeUtente);
...
<BODY onload="initialize()">
	<form name="frmDati" action="lancio_rep_swn_2_cl.jsp" method="post">
