...
<%=StaticMessages.getMessage(3006,"verifica_batch_prefatturazione_cl.jsp")%>
</logtag:logData>
...
String strNameFirstPage = "verifica_batch_prefatturazione_cl.jsp";
strTitle = "Verifica Batch Prefatturazione";
strCodeFunz = StaticContext.RIBES_TOOL_PREFATTURAZIONE;
strImageTitle =StaticContext.PH_TOOL_PREF_BATCH_PREFATTURAZIONE_IMAGES;
%>
			...
			<table width="90%" cellspacing="1" align="center">
				<%
					...
					// carico il vettore
					  ...
					  {
					    vctBatch = (Vector)remoteEnt_Batch_PRE.getElabBatchVerificaPreFatt(strCodeFunz);
					    if (vctBatch!=null)
				...
				if(vctBatch.size()!=0){%>
					...
					<tr bgcolor="<%=StaticContext.bgColorTabellaForm%>">
						...
						<td height="20" class="textB">Fase</td>
					</tr>
				<%}else{%>
					<tr bgcolor="<%=StaticContext.bgColorTabellaForm%>">
				      ...
				      <%
					   ...
					   for(i=((pagerPageNumber.intValue()-1)*intRecXPag);((i < intRecTotali) && (i < pagerPageNumber.intValue()*intRecXPag));i++)	
				       {
							...
							<tr bgcolor="<%=bgcolor%>">
								...
								<td height="20" class="text">&nbsp;<%=Misc.nh(lobj_Batch.getDESC_ANAG_BATCH_PRE())%></td>
							</tr>
<%						}%>
							<tr>
