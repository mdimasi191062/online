...
<%=StaticMessages.getMessage(3006,"ElabTariffaXContrSp.jsp")%>
</logtag:logData>
...
//dati passati dal pannello chiamante: inizio
  String cod_contr = request.getParameter("contrSelez");
  if (cod_contr==null) cod_contr=request.getParameter("cod_contr");
  if (cod_contr==null) cod_contr=(String)session.getAttribute("cod_contr");

  String des_contr = request.getParameter("des_contr");
  if (des_contr==null) des_contr=request.getParameter("des_contr");
  if (des_contr==null) des_contr=(String)session.getAttribute("des_contr");
  //System.out.println("9'' -----> des_contr "+des_contr);     
 ...
 */
  ...
  //System.out.println("4 -----> tipo_pannello "+tipo_pannello);    
  ...
  //System.out.println("5 -----> cod_tar "+cod_tar);      
  ...
  //System.out.println("8 -----> prog_tar "+prog_tar);        
...
// //System.out.println("31 -----> act "+(String)session.getAttribute("act"));    
 ...
 String des_tipo_contr = request.getParameter("des_tipo_contr");
  ...
  //System.out.println("9 -----> cod_tipo_contr "+cod_tipo_contr);          

  

  //SIMULAZIONE
  //des_contr = "DESCRIZIONE CONTRATTO";
...
// //System.out.println("1) hidFlagSys "+hidFlagSys);  
  ...
  //System.out.println("20 --> hidFlagSys "+hidFlagSys);  
...
utilizzato poi da CAUS_FAT_LEGGI_DET*/
  ...
  //System.out.println("7 -----> cod_tipo_caus "+cod_tipo_caus);      
...
//Valeria inizio 07-10-02
  ...
  //System.out.println("7 -----> caricaLista "+caricaLista);      
//Valeria inizio 07-10-02  
...
//  if (importo_tariffa!=null) importo_tariffa=importo_tariffa.replace('.',',');  
  //System.out.println("26 -----> importo_tariffa "+importo_tariffa);                  
  ...
  //System.out.println("26 -----> descrizione_tariffa "+descrizione_tariffa);                  
  ...
  //System.out.println("13 -----> data_fine_tariffa "+data_fine_tariffa);                  
  ...
  //System.out.println("16 -----> data_inizio_tariffa "+data_inizio_tariffa);                  
  //questa è la data fine tariffa che deve essere inserita dall'utente e passata al pannello quando si
  //preme "aggiorna": 
...
//dati da passare alla Servlet quando viene premuto "Disattiva"/"Aggiorna": inizio
  ...
  //System.out.println("10 -----> data_ini_val_of "+data_ini_val_of);            
  ...
  //System.out.println("11 -----> data_ini_val_ass_ofps "+data_ini_val_ass_ofps);              
//dati da passare alla Servlet quando viene premuto "Disattiva": fine  


//modifica opzioni 27-02-03 inizio
...
//modifica opzioni 27-02-03 fine
...
//per la verifica di l'attivazione del pannello 
...
<EJB:useHome id="homeTariffaXContr" type="com.ejbBMP.TariffaXContrBMPHome" location="TariffaXContrBMP" />
<HTML>
<HEAD>
...
{
//	     window.alert("ACT --> "+document.ElabTariffaXContrSp.act.value);       
<%
  if  (act!=null)
  {
...
<%  
       act = null;
%>
       document.ElabTariffaXContrSp.act.value="";
	     //window.alert("ACT --> "+document.ElabTariffaXContrSp.act.value);       
<%       
...
//function trasformaImporto(importo)
//{
//  var new_importo = "";
//  var i = 0;
//  if (importo.length != 0)
//  {
//    for (i=0; importo.length > i; i++)
//    {
//      if (importo.substr(i,1)!=".")
//        new_importo = new_importo + importo.substr(i,1);
//    }
//  }
//  return new_importo;
//}
...
function ONCANCELLA()
{
  if (confirm("Si conferma la cancellazione della tariffa?"))
  {
   ...
   //10-03-03-inizio
   Enable(document.ElabTariffaXContrSp.importo_tariffa); 
   Enable(document.ElabTariffaXContrSp.data_inizio_tariffa); 
   Enable(document.ElabTariffaXContrSp.descrizione_tariffa);                
   Enable(document.ElabTariffaXContrSp.data_ini_val_of);
   Enable(document.ElabTariffaXContrSp.data_ini_val_ass_ofps);
   Enable(document.ElabTariffaXContrSp.nome_utente);
   //10-03-03-fine

   
    document.ElabTariffaXContrSp.act.value = "cancella";
    Enable(document.ElabTariffaXContrSp.cod_tar);
    Enable(document.ElabTariffaXContrSp.prog_tar);
    Enable(document.ElabTariffaXContrSp.act);
    Enable(document.ElabTariffaXContrSp.cod_tipo_caus);    
    Enable(document.ElabTariffaXContrSp.des_tipo_contr);        
    Enable(document.ElabTariffaXContrSp.des_PS); 
    Enable(document.ElabTariffaXContrSp.hidFlagSys);     
    Enable(document.ElabTariffaXContrSp.caricaCausale);                                     
    Enable(document.ElabTariffaXContrSp.caricaLista);                                         
    //Valeria fine 07-10-02
    Enable(document.ElabTariffaXContrSp.tipo_importo);
    Enable(document.ElabTariffaXContrSp.des_contr);
    //Valeria fine 07-10-02

    //modifiche opzioni 27-02-03 inizio
    Enable(document.ElabTariffaXContrSp.caricaOpzione);   
    Enable(document.ElabTariffaXContrSp.opzioneSelez);   
    //modifiche opzioni 27-02-03 fine
    
    document.ElabTariffaXContrSp.action='<%=request.getContextPath()%>/servlet/TariffaXContrCntl';
    document.ElabTariffaXContrSp.submit();
  }
...
{ 
  ...
  if (document.ElabTariffaXContrSp.data_inizio_tariffa.value!=null && document.ElabTariffaXContrSp.data_inizio_tariffa.value!=""){
    /*if (document.ElabTariffaXContrSp.data_inizio_tariffa.value.substring(2,0)=="01") //(2)
    {*/
      //prendo la max tra data_of e data_ofps (5)
      //alert ("data_of = "+document.ElabTariffaXContrSp.data_ini_val_of.value);
      //alert ("data_ofps = "+document.ElabTariffaXContrSp.data_ini_val_ass_ofps.value);
      //alert ("data_ini = "+document.ElabTariffaXContrSp.data_ini_val_of.value);
      data_of = trasformaData(document.ElabTariffaXContrSp.data_inizio_tariffa.value);
      data_ofps = trasformaData(document.ElabTariffaXContrSp.data_ini_val_ass_ofps.value);
      data_ini = trasformaData(document.ElabTariffaXContrSp.data_inizio_tariffa.value);
...
{
  importo_ok = true;
  if (document.ElabTariffaXContrSp.importo_tariffa.value!=null && document.ElabTariffaXContrSp.importo_tariffa.value!="")
  {
       //alert("importo_1 "+ document.ElabTariffaXContrSp.importo_tariffa.value);
       //Valeria inizio 14-10-02
       //istruzione commentata perchè non serve più
       //document.ElabTariffaXContrSp.importo_tariffa.value = trasformaImporto(document.ElabTariffaXContrSp.importo_tariffa.value);
       //Valeria fine 14-10-02
       
       //alert("importo_2 "+ document.ElabTariffaXContrSp.importo_tariffa.value);       
       //ritorno=CheckNum(document.ElabTariffaXContrSp.importo_tariffa.value,12,4,false);       
       //window.alert("importo_tariffa --> "+document.ElabTariffaXContrSp.importo_tariffa.value + "new_importo --> "+new_importo);
       ritorno=CheckNum(document.ElabTariffaXContrSp.importo_tariffa.value,16,4,false);              
...
function ONANNULLA()
{
  ...
  document.ElabTariffaXContrSp.importo_tariffa.value=importo_tariffaStart;
  document.ElabTariffaXContrSp.descrizione_tariffa.value=descrizione_tariffaStart;
  document.ElabTariffaXContrSp.data_inizio_tariffa.value=data_inizio_tariffaStart;
 }

function ONAGGIORNA()
{
     //window.alert(document.ElabTariffaXContrSp.importo_tariffa.value);
     if (document.ElabTariffaXContrSp.descrizione_tariffa.value!=null && document.ElabTariffaXContrSp.descrizione_tariffa.value!="")
     {
        if (controllaImportoXAggiorna()==true && controllaDataXAggiorna()==true)
        {
             //si chiama la servlet
             if (confirm("Si conferma l'aggiornamento della tariffa?"))
             {
               ...
               document.ElabTariffaXContrSp.act.value="aggiorna";
...
function ONDISATTIVA()
{
   ...
   Disable(document.ElabTariffaXContrSp.data_fine_tariffa);

   if (document.ElabTariffaXContrSp.data_fine_tariffa.value!=null && document.ElabTariffaXContrSp.data_fine_tariffa.value!="")
   {
      //controllo che data_inizio_tariffa sia minore di data_fine_tariffa
      if (document.ElabTariffaXContrSp.data_inizio_tariffa_dis.value!=null && document.ElabTariffaXContrSp.data_inizio_tariffa_dis.value!="")
      {
          data1 = trasformaData(document.ElabTariffaXContrSp.data_inizio_tariffa_dis.value);
          data2 = trasformaData(document.ElabTariffaXContrSp.data_fine_tariffa.value);
          if (data1 >= data2) 
          {
          ...
          else
          {
            ...
            {
              //elaborazione possibile
              Enable(document.ElabTariffaXContrSp.data_fine_tariffa);
              
              document.ElabTariffaXContrSp.act.value="disattiva";

              Enable(document.ElabTariffaXContrSp.act);
              Enable(document.ElabTariffaXContrSp.cod_tar);
              Enable(document.ElabTariffaXContrSp.prog_tar);
              Enable(document.ElabTariffaXContrSp.cod_PS);            
              Enable(document.ElabTariffaXContrSp.cod_of);    
              Enable(document.ElabTariffaXContrSp.data_ini_val_of);    
              Enable(document.ElabTariffaXContrSp.data_ini_val_ass_ofps);    
              Enable(document.ElabTariffaXContrSp.cod_contr);    
              Enable(document.ElabTariffaXContrSp.cod_tipo_caus);   
              Enable(document.ElabTariffaXContrSp.caricaCausale);                                               
              Enable(document.ElabTariffaXContrSp.caricaLista);                    
              Enable(document.ElabTariffaXContrSp.des_tipo_contr);                 
...
{
  if (document.ElabTariffaXContrSp.tipo_pannello.value=="DIS")
    document.ElabTariffaXContrSp.data_fine_tariffa.value="";
  else if (document.ElabTariffaXContrSp.tipo_pannello.value=="AGG")
    document.ElabTariffaXContrSp.data_inizio_tariffa.value="";
  else return;
  return;
...
{
// window.alert("rd_fisso --> "+ document.ElabTariffaXContrSp.rd_fisso.checked); 
// window.alert("rd_variabile --> "+ document.ElabTariffaXContrSp.rd_variabile.checked);
 
 //window.alert("stato radio variabile --> "+ "<%=stato_radio_variabile%>"); 
 if (Navigator)
 {
  if (oggetto=="fisso") //cliccato su fisso
  {
    document.ElabTariffaXContrSp.rd_variabile.focus();
    if (document.ElabTariffaXContrSp.rd_variabile.checked == false) //ho cliccato su rd_fisso che era gia' true
    {
      document.ElabTariffaXContrSp.rd_variabile.checked = false;
      document.ElabTariffaXContrSp.rd_fisso.checked = true;
    }
    else
    {
      document.ElabTariffaXContrSp.rd_variabile.checked = true;
      document.ElabTariffaXContrSp.rd_fisso.checked = false;
    }
  }
  else //cliccato su variabile
  {
    document.ElabTariffaXContrSp.rd_fisso.focus();
    
    if (document.ElabTariffaXContrSp.rd_fisso.checked == false) //ho cliccato su rd_variabile che era gia' true
    {
      document.ElabTariffaXContrSp.rd_variabile.checked = true;
      document.ElabTariffaXContrSp.rd_fisso.checked = false;
    }
...
{ 
  ...
  //  window.alert("disabilita"+oggetto);
}
...
<form name="ElabTariffaXContrSp" id="ElabTariffaXContrSp" method="get" onsubmit="return check_form();" action='<%=request.getContextPath()%>/servlet/TariffaXContrCntl'>
<input type="hidden" name=act id=act value="<%=act%>"> <!--10-03-03 -->
 <%
  //INIZIO_2
  OggFattBMPPK pkOggFatt = new OggFattBMPPK();
  TariffaBMPPK pkTariffa = new TariffaBMPPK();  
  TariffaXContrBMPPK pkTariffaXContr = new TariffaXContrBMPPK();      
...
//riga commentata perchè il dato viene acquisito dal chiamante
  //String cod_tipo_caus;         //codice tipo causale
//Valeria fine 30-09-02  
  ...
  //System.out.println("AVVIO DEL PANNELLO CON TIPO PANNELLO ---> "+tipo_pannello);
  //inizializzo le date
  ...
  //System.out.println("DOPO LE IMPOSTAZIONI");
  //System.out.println("data_inizio_tariffa ----> "+data_inizio_tariffa);
  //System.out.println("data_fine_tariffa ---> "+data_fine_tariffa);
  ...
  {
%>  
    <!--chiamo TARIFFA_X_CONTR_VER_DISATTIVA-->
    <EJB:useBean id="remote_14" type="com.ejbBMP.TariffaXContrBMP" value="<%=homeTariffaXContr.findTarXContrVerDisatt(cod_tar, prog_tar)%>" scope="session"></EJB:useBean>
<%
      num_tar_disattive = remote_14.getNumTarDisattive();

      //System.out.println("NUM_TAR_DISATTIVE *********** "+num_tar_disattive.intValue());
      ...
      {
%>      
        <!--chiamo TARIFFA_X_CONTR_VER_PROVV-->
        <EJB:useBean id="remote_15" type="com.ejbBMP.TariffaXContrBMP" value="<%=homeTariffaXContr.findTarXContrVerProvv(cod_tar, prog_tar)%>" scope="session"></EJB:useBean>
<%
          num_tar_provvisor = remote_15.getNumTarProvvisor();

         //System.out.println("NUM_TAR_PROVVISOR *********** "+num_tar_provvisor.intValue());
...
%>
    ...
    <!--chiamo UNITA_MISURA_DETT_X_CONTR-->
    <EJB:useBean id="remote_11" type="com.ejbBMP.TariffaXContrBMP" value="<%=homeTariffaXContr.findUnitaMisuraDettXTarXContr(cod_tar, prog_tar)%>" scope="session"></EJB:useBean>
<%
    ...
    //System.out.println("Prima di chiamare OggFatt, cod_of --> "+cod_of+" hidFlagSys= " +hidFlagSys);
%>
...
<%
    ...
    //SIMULAZIONE
    /*desc_classe_of = "desc_classe_of";
    code_classe_of = "5";*/
%>            

    <!--chiamo TARIFFA_X_CONTR_DETTAGLIO-->

<%
    pkTariffaXContr.setCodTar(cod_tar);
    pkTariffaXContr.setProgTar(prog_tar);    
    pkTariffaXContr.setCodContr(cod_contr);        

    //System.out.println("***************+PRIMA DI CHIAMARE TARIFFA_DETTAGLIO******************");
    //System.out.println("cod_tar --> "+cod_tar +" e in PK "+ pkTariffaXContr.getCodTar() );
    //System.out.println("prog_tar --> "+prog_tar);    
    //System.out.println("cod_contr --> "+cod_contr);    
%>
    <EJB:useBean id="remote_13" type="com.ejbBMP.TariffaXContrBMP" value="<%=homeTariffaXContr.findByPrimaryKey(pkTariffaXContr)%>" scope="session"></EJB:useBean>
<%
...
//    //System.out.println(remote_13.getImpTar().doubleValue());   
//    //System.out.println((importo_tariffa==null));
//    //System.out.println("valore di importo_tariffa --> "+importo_tariffa);
//    //System.out.println("remote_13.getDataIniValOf()");               
     ...
     data_ini_val_ass_ofps = remote_13.getDataIniValAssOfPs();   
      ...
      if (remote_13.getCausFatt()!=null)
         cod_tipo_caus = remote_13.getCausFatt(); //10-03-03

         
     //Valeria inizio 30-09-02 
     //linea eliminata perchè il cod_tipo_caus mi viene passato
     //cod_tipo_caus = remote_13.getCodTipoCaus();
     //Valeria fine 30-09-02      

     //if (importo_tariffa!=null) importo_tariffa=importo_tariffa.replace('.',',');         

//SIMULAZIONE
     /*if (data_ini_val_of==null) 
      data_ini_val_of = "10/09/2002";*/
//SIMULAZIONE
     /*if (data_ini_val_ass_ofps==null) 
      data_ini_val_ass_ofps = "30/09/2002";*/
//SIMULAZIONE
     /*if (data_inizio_tariffa==null) 
      data_inizio_tariffa = "01/09/2002";*/
     ...
     { //IF__5 
       //GI_11
       //System.out.println("chiamo CAUS_FAT_LEGGI_DETT");
%>
...
<%    
        ...
        //System.out.println("desc_tipo_caus --> "+ desc_tipo_caus);                 
        //GI_11_FINE  
     }
     else
     {
         //System.out.println("pongo desc_tipo_caus = null");
          //GI_12
          desc_tipo_caus = null; // la pongo a null per le verifiche successive
          //GI_12_FINE
     }  //ENDIF__5

              //System.out.println("aconn.jsp="+StaticContext.DSNAME);
...
%> 
  ...
  </table>  <!--TABELLA_1_FINE quando apri_pannello=false-->
<%       
             ...
             <td>
               <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorTabellaForm%>"> <!--TABELLA_7-->
                 <tr>
                   ...
                   <td class="textb"  align="left" width="50%">&nbsp;Contratto</td>
                 </tr>
                 <tr>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorTabellaForm%>"> <!--TABELLA_7_1-->
                 <tr>
                   <td class="textB" align="left" >&nbsp;P/S</td>
                 </tr>
                <tr>
                ...
                </table> <!--TABELLA_7_1_fine-->
                
                <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorTabellaForm%>"> <!--TABELLA_7_2-->
                <tr>
              ...
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="1" align='center'> <!--TABELLA_12-->
                    <tr>
                      <td class="textB" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>" align="left" width="17%">&nbsp;Tipo importo</td> 
                      <td class="textB" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>" width="15%">
<%
                      //Valeria inizio 07-10-02
                      if (tipo_importo!=null)
                      {
...
%>
                        ...
                        document.ElabTariffaXContrSp.rd_fisso.checked = <%=stato_radio_fisso%>;
                        document.ElabTariffaXContrSp.rd_variabile.checked = <%=stato_radio_variabile%>;

                        Disable(document.ElabTariffaXContrSp.rd_variabile);
                        Disable(document.ElabTariffaXContrSp.rd_fisso);                        
              ...
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="1" align='center'> <!--TABELLA_14-->
                    <tr>
                      <td class="textB" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>" align="left" width="20%">&nbsp;Descrizione tariffa
                      </td>
<%
...
%>
                          ...
                          <input type="hidden" name=descrizione_tariffa id=descrizione_tariffa value=<%=descrizione_tariffa%>><!--10-03-03-->                       
                          <%=descrizione_tariffa%>&nbsp; 
                          </td>                          
<%
                      }  
...
<%
                      if (tipo_pannello.equalsIgnoreCase("AGG")) //il campo e' textfield abilitato
                      {
                          ...
                          //System.out.println("IMPORTO TARIFFA ----> "+importo_tariffa);
%>                                  
...
%> 
                          ...
                          <%=importo_tariffa_a%>&nbsp;                          
                          <input type="hidden" name=importo_tariffa id=importo_tariffa value=<%=importo_tariffa%>><!--10-03-03-->
                          </td>                          
...
%>
                        ...
                        <SCRIPT LANGUAGE='Javascript'>
                        Disable(document.ElabTariffaXContrSp.data_inizio_tariffa);
                        </SCRIPT>
                        

                        <td width=50%>  

                          <a href="javascript:showCalendar('ElabTariffaXContrSp.data_inizio_tariffa','<%=data_inizio_mmdd%>');" 
...
%>
                          ...
                          <input type="hidden" name=data_inizio_tariffa id=data_inizio_tariffa value=<%=data_inizio_tariffa%>> <!--10-03-03-->
                          <%=data_inizio_tariffa%>&nbsp;                                           
                          </td>                          
...
<%
                      ...
                      if (tipo_pannello.equalsIgnoreCase("DIS")) //solo in questo caso si mostra la data fine validita
                      {
%>                                  
                        ...
                        <td width=50%>  
                          ...
                          <a href="javascript:showCalendar('ElabTariffaXContrSp.data_fine_tariffa','<%=data_fine_mmdd%>');" 
                          onMouseOver="javascript:showMessage('seleziona1'); return true;" onMouseOut="status='';return true">
                          <img name='calendar_ini' src="../../common/images/body/calendario.gif" border="no"></a>                                    
...
%>                                  
      ...
      <tr>
        ...
        <input type="hidden" name=cod_contr id=cod_contr value="<%=cod_contr%>">   
        <!--Valeria inizio 30-09-02-->    
        <!--aggiunto cod_tipo_caus per la chiamata a ListaTariffeSp-->
...
<%
            if (tipo_pannello.equalsIgnoreCase("DIS"))
            {
            //System.out.println("bottone DISATTIVAZIONE");
%>
...
<%
            ...
            {
%>
              <sec:ShowButtons PageName="ELABTARXCONTRSP_AGG" /> 
               <Script language='javascript'>
...
<%
            ...
            {
%>
              <sec:ShowButtons PageName="ELABTARXCONTRSP_CAN" />                
<%
            }
%>
