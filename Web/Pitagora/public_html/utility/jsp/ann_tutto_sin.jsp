...
<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
  <%=StaticMessages.getMessage(3006,"ann_tutto_sin.jsp")%>
</logtag:logData>
...
<%
  int esito = 0;
  String messaggio = "";
  String codeInventario = Misc.nh(request.getParameter("txtCodiceInventario"));
  ...
  if(!operazione.equals(""))
  {
      ...
      esito = remoteCtr_Utility.annullaTuttoSingolo(codeInventario,"H3",tracciamento);

      if(esito==1)
        messaggio = "Annullamento effettuato correttamente.";
      else
        if(esito==2)
            messaggio = "Codice inventario non presente in inventario.";
        else    
...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function ONESEGUI()
{ 
        if(document.frmDati.txtCodiceInventario.value==''){
            alert('Digitare il Codice Inventario.');
            document.frmDati.txtCodiceInventario.focus();
        return;
        }
        if(document.frmDati.txtTracciamento.value==''){
            alert('Digitare il codice Tracciamento.');
            document.frmDati.txtTracciamento.focus();
        return;
        }         
        
        document.frmDati.action = "ann_tutto_sin.jsp?operazione=1";
...
<tr>
  <td>
        ...
        <tr height="35px">
            <td class="textB">Codice Inventario: </td>
            <td  class="text">
                <input class="text" type='text' name='txtCodiceInventario' obbligatorio="si" value=''  >
            </td>
       </tr>
        <tr height="35px">
