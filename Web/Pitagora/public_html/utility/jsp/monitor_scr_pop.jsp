...
<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
  <%=StaticMessages.getMessage(3006,"monitor_scr_pop.jsp")%>
</logtag:logData>
...
<%
  ...
  Vector vctScartiPopolamento = null;
  if(!operazione.equals(""))
  {
        System.out.println("operazione 1 " );
        vctScartiPopolamento = remoteCtr_Utility.getScartiPopolamento(datarif);
   }
  %>
<html>
<head>
...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function ONAGGIORNA()
{ 
        document.frmDati.action = "monitor_scr_pop.jsp?operazione=1";
...
<table width="100%">
<td width=5%></td>
<td width=95%><img src="../images/ScartiPopolamento.gif" alt="" border="0">
</td>
</table>
...
<%
  ...
  if(operazione.equals("")) {
    vctElabBatch = remoteCtr_Utility.getScartiPopolamento("01/01/1900");
    }
  else
    {   
    vctElabBatch = remoteCtr_Utility.getScartiPopolamento(datarif);
  }
  String strNameFirstPage = "monitor_scr_pop.jsp";
...
<!--  <tr>
    <td><img src="../images/ScartiPopolamento.gif" alt="" border="0"></td>
  </tr>
-->
<tr height="20">
  <td>
...
<TR valign="top">
    ...
    <table width="100%" cellspacing="1" align="center">
      <%if(vctElabBatch.size()!=0){%>
        <tr class="rowHeader" height="20" align="center">
            <td >Code<BR>Tipo<BR>Contr</td>      
            <td >Desc<BR>Tipo<BR>Contr</td>      
            <td >Code Dett<BR>Scarto</td>      
  ...
  out.flush();
  if(operazione.equals(""))    {
    vctScartiPopolamento = null;
    vctScartiPopolamento = remoteCtr_Utility.getScartiPopolamento("01/01/1900"); 
  }
  if (request.getParameter("cboNumRecXPag")!=null)
    intRecXPag = Integer.parseInt(request.getParameter("cboNumRecXPag"));
        String classRow = "row2"; 
        //for(int i=0;i<vctElabBatch.size();i++){
        for(int i=((pagerPageNumber.intValue()-1)*intRecXPag);((i < vctScartiPopolamento.size()) && (i < pagerPageNumber.intValue()*intRecXPag));i++){
           classRow = classRow.equals("row2") ? "row1" : "row2";
           DB_ScartiPopolamento objUtility = (DB_ScartiPopolamento)vctScartiPopolamento.get(i);
           %>
           <TR>
            ...
            <td  class="<%=classRow%>" align="center"><%=objUtility.getDESCRIZIONE_SCARTO()%></td> 
...
<tr height="28" class="text">
  <td >
    <pg:index>
       Risultati Pag.
    <pg:prev> 
    <A HREF="javaScript:goPage('monitor_scr_pop.jsp')" onMouseOver="status='Go to Page <%=pageNumber%>';return true" onMouseOut="status='';return true">[<< Prev]</A>
    </pg:prev>
    <pg:pages>
      <%if (pageNumber == pagerPageNumber){%>
             <b><%=pageNumber%></b>&nbsp;
      <%}else{%>
              <A HREF="javaScript:goPage('monitor_scr_pop.jsp')" onMouseOver="status='Go to Page <%=pageNumber%>';return true" onMouseOut="status='';return true"><%= pageNumber %></A>&nbsp;
      <%}%>
    </pg:pages>
    <pg:next>
      <A HREF="javaScript:goPage('monitor_scr_pop.jsp')" onMouseOver="status='Go to Page <%=pageNumber%>';return true" onMouseOut="status='';return true">[Next >>]</A>
    </pg:next>
    </pg:index>
    </pg:pager>
  </td>
