...
<logtag:logData id="<%= ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName() %>">
  <%=StaticMessages.getMessage(3006,"mon_sca_val.jsp")%>
</logtag:logData>
...
<%
  ...
  Vector vctScartiVal = null;
  if(!operazione.equals(""))
  {
        System.out.println("operazione 1 " );
        vctScartiVal = remoteCtr_Utility.getScartiVal(datarif);
   }
  %>
<html>
<head>
...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function ONAGGIORNA()
{ 
        document.frmDati.action = "mon_sca_val.jsp?operazione=1";
...
<table width="100%">
<td width=5%></td>
<td width=95%><img src="../images/ScartiVal.gif" alt="" border="0">
</td>
</table>
...
<%
  ...
  if(operazione.equals("")) {
    vctElabBatch = remoteCtr_Utility.getScartiVal("01/01/1900");
    }
  else
    {   
    vctElabBatch = remoteCtr_Utility.getScartiVal(datarif);
  }
  
  String strNameFirstPage = "monitor_sca_val.jsp";
...
<!--  <tr>
    <td><img src="../images/ScartiVal.gif" alt="" border="0"></td>
  </tr>
-->
<tr height="20">
  <td>
...
<tr height="20">
  <td>
    <table width="100%" border="1" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>" bordercolor="<%=StaticContext.bgColorHeader%>">
      <tr>
        <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">Riepilogo Scarti di Valorizzazione</td>
...
<TR valign="top">
  ...
  if(operazione.equals(""))    {
    vctScartiVal = null;
    vctScartiVal = remoteCtr_Utility.getScartiVal("01/01/1900"); 
  }
  ...
  if (request.getParameter("cboNumRecXPag")!=null)
    intRecXPag = Integer.parseInt(request.getParameter("cboNumRecXPag"));
        ...
        for(int i=((pagerPageNumber.intValue()-1)*intRecXPag);((i < vctScartiVal.size()) && (i < pagerPageNumber.intValue()*intRecXPag));i++){
           classRow = classRow.equals("row2") ? "row1" : "row2";
           DB_ScartiVal objUtility = (DB_ScartiVal)vctScartiVal.get(i);
           %>
           <TR>
            ...
            <td  class="<%=classRow%>" align="center"><%=objUtility.getCODE_MOTIVO_SCARTO()%></td> 
            <td  class="<%=classRow%>" align="center"><%=objUtility.getDESC_MOTIVO_SCARTO()%></td> 
            <td  class="<%=classRow%>" align="center"><%=objUtility.getDATA_CREAZ_MODIF().substring(0,10)%></td>
...
<tr height="28" class="text">
  <td >
    <pg:index>
       Risultati Pag.
    <pg:prev> 
    <A HREF="javaScript:goPage('mon_sca_val.jsp')" onMouseOver="status='Go to Page <%=pageNumber%>';return true" onMouseOut="status='';return true">[<< Prev]</A>
    </pg:prev>
    <pg:pages>
      <%if (pageNumber == pagerPageNumber){%>
             <b><%=pageNumber%></b>&nbsp;
      <%}else{%>
              <A HREF="javaScript:goPage('mon_sca_val.jsp')" onMouseOver="status='Go to Page <%=pageNumber%>';return true" onMouseOut="status='';return true"><%= pageNumber %></A>&nbsp;
      <%}%>
    </pg:pages>
    <pg:next>
      <A HREF="javaScript:goPage('mon_sca_val.jsp')" onMouseOver="status='Go to Page <%=pageNumber%>';return true" onMouseOut="status='';return true">[Next >>]</A>
    </pg:next>
    </pg:index>
    </pg:pager>
  </td>
