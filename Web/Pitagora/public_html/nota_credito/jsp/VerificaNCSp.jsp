...
<%=StaticMessages.getMessage(3006,"VerificaNCSp.jsp")%>
</logtag:logData>
...
Collection oggAbort;
%>
<EJB:useHome id="homeNC" type="com.ejbBMP.ElaborBatchBMPHome" location="ElaborBatchBMP" />
...
function ChangeSel(codice, codeStato)
{
  ...
  document.verificaForm.code_elabNC.value=codice;
  document.verificaForm.code_statoNC.value=codeStato;
  
}
function ONACCOUNT_ELAB()
{

    document.verificaForm.action="NCLstAccElabSp.jsp" ;
    document.verificaForm.submit();
}
...
function ONAGGIORNA()
{
    ...
    document.verificaForm.action="VerificaNCSp.jsp" ;
    document.verificaForm.submit();  
}
</SCRIPT>
...
<%
   ...
   //04-01-03 viti
       ...
       datiBatch=remoteBatch.getCodeFunzFlag(cod_tipo_contr,"CN",null,"S");
       if (datiBatch!=null)
       {
           flagTipoContr=  (new Integer(datiBatch.getFlagTipoContr())).toString();
           codeFunzBatch = datiBatch.getCodeFunz();
       }    
       oggAbort = homeNC.findLstVer(codeFunzBatchNC,cod_tipo_contr);
    }
    ...
    {    
        ...
        code_statoNC=elab.getCodeStato();
        if(code_statoNC!=null)
            session.setAttribute("code_statoNC",code_statoNC);
        code_elabNC=elab.getCodeElab();
        if(code_elabNC!=null)
            session.setAttribute("code_elabNC",code_elabNC);
        sizeLst=String.valueOf(oggAbort.size());
    }
     else sizeLst="0";
%>

<form name="verificaForm" method="get" action="VerificaNCSp.jsp">
