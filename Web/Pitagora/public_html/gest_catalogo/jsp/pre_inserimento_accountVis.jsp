...
<%=StaticMessages.getMessage(3006,"pre_inserimento_accountVis.jsp")%>
</logtag:logData>
...
<%
  ...
  String esito = "";
  String messaggio = "";
  DB_CAT_Account newAccount;
  
  String strCodeAcc = Misc.nh(request.getParameter("strCodeAcc"));
  String strDescAcc = Misc.nh(request.getParameter("strDescAcc"));
  String strCodeServ = Misc.nh(request.getParameter("strCodeServ"));
  String strCodeServLogico = Misc.nh(request.getParameter("strCodeServLogico"));
  String strCodeGest = Misc.nh(request.getParameter("strCodeGest"));
  String codeGestSap = Misc.nh(request.getParameter("strCodeGestSap"));
  String strFlagClasSconto = (Misc.nh(request.getParameter("strFlagClasSconto"))).equals("true")?"S":"N";
  String strFlagAccFitt = (Misc.nh(request.getParameter("strFlagAccFitt"))).equals("true")?"S":"N";  
  String strFlagFatt = (Misc.nh(request.getParameter("strFlagFatt"))).equals("true")?"S":"N";
  String strFlagInterim = (Misc.nh(request.getParameter("strFlagInterim"))).equals("true")?"S":"N";
  String strFlagInvSap = (Misc.nh(request.getParameter("strFlagInvSap"))).equals("true")?"S":"N";  
  String divAcc = Misc.nh(request.getParameter("strDIVAcc"));
  String dfvAcc = Misc.nh(request.getParameter("strDFVAcc"));
  ...
  if(!strCodeAcc.equals("") && intAggiornamento != 0){
    newAccount = new DB_CAT_Account();
    newAccount.setCODE_ACCOUNT(strCodeAcc);
    newAccount.setDESC_ACCOUNT(strDescAcc);
    newAccount.setCODE_SERVIZIO(strCodeServ);
//AP Aggiunta gestione servizio logico
    newAccount.setCODE_SERVIZIO_LOGICO(strCodeServLogico);
    newAccount.setCODE_GESTORE(strCodeGest);
    newAccount.setCODE_GESTORE_SAP(codeGestSap);
    newAccount.setTIPO_FLAG_CLAS_SCONTO(strFlagClasSconto);
    newAccount.setTIPO_FLAG_FITTZ(strFlagAccFitt);
    newAccount.setTIPO_FLAG_FATTRB(strFlagFatt);
    newAccount.setDATA_INIZIO_VALID(divAcc);
    newAccount.setDATA_INIZIO_VALIDITA_SAP(divAcc);    
    newAccount.setDATA_FINE_VALID(dfvAcc);
    newAccount.setDATA_FINE_VALIDITA_SAP(dfvAcc);    
    newAccount.setFLAG_INTERIM(strFlagInterim);
    newAccount.setFLAG_INVIO_SAP(strFlagInvSap);
    newAccount.setFLAG_MODIF(strFlagModificaDescrizione);

    /* INSERIMENTO */
    if(intAggiornamento==1)
    {
      esito = remote_EntCatalogo.insAccount(newAccount);
      //esito = remote_EntCatalogo.insAccount(newAccount);    
      if(esito.equals("")){
        messaggio = "Inserimento nuovo Account effettuato correttamente.";
      }else{
        messaggio = esito;
      }
    }
    ...
    {
      esito = remote_EntCatalogo.aggiorna_Account(newAccount);
    ...
    {
      esito = remote_EntCatalogo.cancella_account(strCodeAcc);
      
      if(esito.equals("")){
        messaggio = "Cancellazione Account effettuata correttamente.";
      }else{
        messaggio = esito;
      }
    }
  ...
  strCodeAcc = Misc.nh(request.getParameter("txtAccount"));
  strDescAcc = Misc.nh(request.getParameter("txtDescAccount"));  
  if(strCodeAcc.equals(""))
    strCodeAcc = "%";
  ...
  {
    accountVector = remote_EntCatalogo.getAccountFiltro(strCodeAcc,strDescAcc);
...
function submitRicerca(typeLoad)
{
  ...
  objForm.action = "pre_inserimento_accountVis.jsp";
  objForm.submit();
}
...
function ONAGGIUNGI() {
  var URL = '';
  URL='pre_inserimento_account.jsp';
  openCentral(URL,'Account','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',700,500);
}

function ONAGGIORNA() {
  var URL = '';
  URL='pre_aggiornamento_account.jsp?code_account='+document.frmDati.strCodeAcc.value;
  openCentral(URL,'Gestori','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',700,500);
}

function ONCANCELLA() {
  if(confirm("Attenzione!!! Si vuole procedere con la cancellazione dell'account selezionato?")){
...
function ONMODIFICADESC() {
  var URL = '';
  URL='pre_aggiornamento_account.jsp?code_account='+ document.frmDati.strCodeAcc.value + '&modifica_descrizione=1';
  openCentral(URL,'Componenti','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',700,500);  
}
...
function setCodeHidden(obj){
  ...
  document.frmDati.strCodeAcc.value=obj.value;
}
</SCRIPT>
</HEAD>
...
<input type="hidden" name="strCodeAcc" value="">
<input type="hidden" name="strDescAcc" value="">
<input type="hidden" name="strCodeServ" value="">
<input type="hidden" name="strCodeServLogico" value="">
<input type="hidden" name="strCodeGest" value="">
<input type="hidden" name="strCodeGestSap" value="">
<input type="hidden" name="strFlagClasSconto" value="">
<input type="hidden" name="strFlagAccFitt" value="">
<input type="hidden" name="strFlagFatt" value="">
<input type="hidden" name="strFlagInterim" value="">
<input type="hidden" name="strFlagInvSap" value="">
<input type="hidden" name="strDIVAcc" value="">
<input type="hidden" name="strDFVAcc" value="">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
				<tr>
					<td>
					  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
						<tr>
						  <td bgcolor="#0A6B98" class="white" valign="top" width="91%">&nbsp;Account</td>
              ...
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                    ...
                    <tr>
                      <td width="20%" class="textB" align="right">Codice Account:&nbsp;</td>
                      <%
                      if(strCodeAcc.equals("%"))
                        strCodeAcc = "";
                      %>
                      <td width="20%" class="text"><input class="text" type='text' name='txtAccount' value='<%=strCodeAcc%>' size='15'></td>
                      <td width="20%" class="textB" align="right" nowrap>Descrizione Account:&nbsp;</td>
                      <%
                      if(strDescAcc.equals("%"))
                        strDescAcc = "";
                      %>
                      <td width="20%" class="text"><input class="text" type='text' name='txtDescAccount' value='<%=strDescAcc%>' size='15'></td>
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                    <tr>
                      <td>
                        <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                          <tr>
                            <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Account Selezionati</td>
                            <td bgcolor="<%=StaticContext.bgColorCellaBianca%>" class="white" align="center" width="9%"><img src="<%=StaticContext.PH_COMMON_IMAGES%>quad_blu.gif"></td>
                          </tr>
                        </table>
                      </td>
                    ...
                    <tr>
                      <td>
                        <table width="100%" align='center' border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td colspan='8' bgcolor="<%=StaticContext.bgColorCellaBianca%>"><img src="<%=StaticContext.PH_COMMON_IMAGES%>pixel.gif" width="1" height='2'></td>
                          </tr>
                          <%
                          int i=0;
                          int j=0;
                          ...
                          <tr>
                            ...
                            <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Fatturabile&nbsp;</td>
                            <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Fittizio&nbsp;</td>
                            <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Interim&nbsp;</td>
                            <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">Gestore Sap&nbsp;</td>
                          </tr>
                          ...
                          <tr class="<%=curSubClass_Row%>">
                            <td class='text'>
                              <input type="radio" name="CodeAccount" value="<%=myelement.getCODE_ACCOUNT()%>" onClick="setCodeHidden(this);">
                              <input type="hidden" name="tipo_sistema_mittente_<%=myelement.getCODE_ACCOUNT()%>" value="<%=myelement.getTIPO_SISTEMA_MITTENTE()%>">
                            </td>
                            
                            <td><%=myelement.getCODE_ACCOUNT()%></td>
                            <td><%=myelement.getDESC_ACCOUNT()%></td>
                            <td><%=myelement.getTIPO_FLAG_FATTRB()%></td>
                            <td><%=myelement.getTIPO_FLAG_FITTZ()%></td>
                            <td><%=myelement.getFLAG_INTERIM()%></td>
                            <td><%=myelement.getDESC_GESTORE()%></td>
                          </tr>
                          <tr>
                            <td colspan='8' bgcolor="<%=StaticContext.bgColorCellaBianca%>"><img src="<%=StaticContext.PH_COMMON_IMAGES%>pixel.gif" width="1" height='2'></td>
                          </tr>
