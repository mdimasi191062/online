...
<%
  ...
  if(!codice.equals("")&& intAggiornamento != 0){
      ...
      //AP Aggiunta modifica gestione descrizione 
      newPrestAgg.setFLAG_MODIF(strFlagModificaDescrizione);

    /* INSERIMENTO */
    if(intAggiornamento==1)
    {
      esito = prestazioni.insPrestAgg(newPrestAgg);
      if(esito.equals("")){
    ...
    {
      esito = prestazioni.aggiorna_prestazione(newPrestAgg);
      
      if(esito.equals("")){
        messaggio = "Aggiornamento Prestazione Aggiuntiva effettuato correttamente.";
      }else{
        messaggio = esito;
      }
    }
    ...
    {
      esito = prestazioni.cancella_prestazione(codice);
      
      if(esito.equals("")){
        messaggio = "Cancellazione Prestazione Aggiuntiva effettuata correttamente.";
      }else{
        messaggio = esito;
      }
    }
  ...
  {
    accountVector = prestazioni.getPrestazioniFiltro(strAccount,strDescAccount);
...
function submitRicerca(typeLoad)
{
  ...
  objForm.action = "pre_inserimento_prestazioniVis.jsp";
  objForm.submit();
}
...
function ONAGGIUNGI() {
  var URL = '';
  URL='pre_inserimento_prestazioni.jsp';
  openCentral(URL,'Prestazioni','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',900,500);
}

function ONAGGIORNA() {
  var URL = '';
  URL='pre_aggiornamento_prestazioni.jsp?code_prestazione='+document.frmDati.strCodePrestAgg.value;
  openCentral(URL,'Prestazioni','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',600,400);
}

function ONCANCELLA() {
  if(confirm("Attenzione!!! Si vuole procedere con la cancellazione della prestazione aggiuntiva selezionata?")){
...
function ONMODIFICADESC() {
  var URL = '';
  URL='pre_aggiornamento_prestazioni.jsp?code_prestazione='+document.frmDati.strCodePrestAgg.value + '&modifica_descrizione=1';
...
function setCodeHidden(obj){
  ...
  document.frmDati.strCodePrestAgg.value=obj.value;
...
<input type="hidden" name="strCodePrestAgg" value="">
<input type="hidden" name="strDescPrestAgg" value="">
<input type="hidden" name="strTipo_caratProd" value="">
<input type="hidden" name="strColocazione" value="">
<input type="hidden" name="strElemTrasm" value="">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
				<tr>
					<td>
					  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
						<tr>
						  <td bgcolor="#0A6B98" class="white" valign="top" width="91%">&nbsp;Prestazioni Aggiuntive</td>
              ...
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                    ...
                    <tr>
                      <td width="20%" class="textB" align="right">Codice Prestazione:&nbsp;</td>
                      <%
                      if(strAccount.equals("%"))
                        strAccount = "";
                      %>
                      <td width="20%" class="text"><input class="text" type='text' name='txtAccount' value='<%=strAccount%>' size='15'></td>
                      <td width="20%" class="textB" align="right" nowrap>Descrizione Prestazione:&nbsp;</td>
                      <%
                      if(strDescAccount.equals("%"))
                        strDescAccount = "";
                      %>
                  ...
                  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                    <tr>
                      <td>
                        <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
                          <tr>
                            <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Prestazioni Selezionati</td>
                            <td bgcolor="<%=StaticContext.bgColorCellaBianca%>" class="white" align="center" width="9%"><img src="<%=StaticContext.PH_COMMON_IMAGES%>quad_blu.gif"></td>
                          </tr>
                        </table>
                      </td>
                      ...
                      <td>
                        <table width="100%" align='center' border="0" cellspacing="0" cellpadding="0">
                          ...
                          String curClassRow = "";
                            ...
                            myelement=(DB_PrestazioneAggiuntiva)accountVector.elementAt(j);
                           ...
                           <tr class="<%=curSubClass_Row%>">
                            <td width=10>
                              <input type="radio" name="CodeFunz" value="<%=myelement.getCODE_PREST_AGG()%>" onClick="setCodeHidden(this);">
                              <input type="hidden" name="tipo_sistema_mittente_<%=myelement.getCODE_PREST_AGG()%>" value="<%=myelement.getTIPO_SISTEMA_MITTENTE()%>">
                            </td>
                           
                            <td><%=myelement.getCODE_PREST_AGG()%></td>
                            <td><%=myelement.getDESC_PREST_AGG()%></td>
                          </tr>
