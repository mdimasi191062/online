...
<%=StaticMessages.getMessage(3006,"pre_inserimento_gestoriVis.jsp")%>
</logtag:logData>

<!-- instanziazione dell'oggetto remoto -->
<EJB:useHome id="home" type="com.ejbSTL.Ent_CatalogoHome" location="Ent_Catalogo" />
<EJB:useBean id="gestore" type="com.ejbSTL.Ent_Catalogo" scope="session">
...
<%
  ...
  String esito = "";
  String messaggio = "";
  DB_Gestori gestoreDB;
  String codeGest = Misc.nh(request.getParameter("strCodeGest"));
  String descGest = Misc.nh(request.getParameter("strDescGest"));
  //String codeGestSap = Misc.nh(request.getParameter("strCodeGestSap"));
  String ivaGest = Misc.nh(request.getParameter("strIvaGest"));
  String divGest = Misc.nh(request.getParameter("strDIVGest"));
  String dfvGest = Misc.nh(request.getParameter("strDFVGest"));

  String strCodeGestore = Misc.nh(request.getParameter("CodeGestore"));
  ...
  if(!codeGest.equals("") && intAggiornamento != 0){
    gestoreDB = new DB_Gestori();
    gestoreDB.setCODE_GESTORE(codeGest);
    gestoreDB.setDESC_GESTORE(descGest);
    gestoreDB.setCODE_PARTITA_IVA(ivaGest);
    gestoreDB.setDATA_INIZIO_VALID(divGest);
    gestoreDB.setDATA_FINE_VALID(dfvGest);
    gestoreDB.setFLAG_MODIF(strFlagModificaDescrizione);

    /* INSERIMENTO */
    if(intAggiornamento==1)
    {
      esito = gestore.insGestore(gestoreDB);
      //esito = gestore.insGestore(gestoreDB,codeGestSap);
      
      if(esito.equals("")){
        messaggio = "Inserimento nuovo Gestore effettuato correttamente.";
      }else{
        messaggio = esito;
      }
    }
    ...
    {
      esito = gestore.aggiorna_gestore(gestoreDB);
      
      if(esito.equals("")){
        messaggio = "Aggiornamento Gestore effettuato correttamente.";
      }else{
        messaggio = esito;
      }
    }
    ...
    {
      esito = gestore.cancella_gestore(strCodeGestore);
      
      if(esito.equals("")){
        messaggio = "Cancellazione Gestore effettuata correttamente.";
      }else{
        messaggio = esito;
      }
    }
  ...
  {
    accountVector = gestore.getGestoriFiltro(strAccount,strDescAccount);
...
function submitRicerca(typeLoad)
{
  ...
  objForm.action = "pre_inserimento_gestoriVis.jsp";
  objForm.submit();
}
...
function ONAGGIUNGI() {
  var URL = '';
  URL='pre_inserimento_gestori.jsp';
  openCentral(URL,'Gestore','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',600,400);
}

function ONAGGIORNA() {
  var URL = '';
  URL='pre_aggiornamento_gestori.jsp?code_gestore='+document.frmDati.strCodeGest.value;
  openCentral(URL,'Gestori','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',600,400);
}

function ONCANCELLA() {
  if(confirm("Attenzione!!! Si vuole procedere con la cancellazione del gestore selezionato?")){
...
function ONMODIFICADESC() {
  var URL = '';
  URL='pre_aggiornamento_gestori.jsp?code_gestore='+ document.frmDati.strCodeGest.value + '&modifica_descrizione=1';
...
function setCodeHidden(obj){
  ...
  document.frmDati.strCodeGest.value=obj.value;
}
...
<input type="hidden" name="strDescGest" value="">
<input type="hidden" name="strCodeGestSap" value="">
<input type="hidden" name="strIvaGest" value="">
<input type="hidden" name="strDIVGest" value="">
<input type="hidden" name="strDFVGest" value="">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
				<tr>
					<td>
					  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
						<tr>
						  <td bgcolor="#0A6B98" class="white" valign="top" width="91%">&nbsp;Gestori</td>
              ...
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                    ...
                    <tr>
                      <td width="20%" class="textB" align="right">Codice Gestore:&nbsp;</td>
                      <%
                      if(strAccount.equals("%"))
                        strAccount = "";
                      %>
                      <td width="20%" class="text"><input class="text" type='text' name='txtAccount' value='<%=strAccount%>' size='15'></td>
                      <td width="20%" class="textB" align="right" nowrap>Descrizione Gestore:&nbsp;</td>
                      <%
                      if(strDescAccount.equals("%"))
                        strDescAccount = "";
                      %>
                      ...
                      <td>
                        <table width="100%" align='center' border="0" cellspacing="0" cellpadding="0">
                          ...
                          <tr>
                            ...
                            <td bgcolor="<%=StaticContext.bgColorTestataTabella%>" class="textB">P.IVA </td>
                          ...
                          {
                            DB_Gestori myelement=new DB_Gestori();
                            myelement=(DB_Gestori)accountVector.elementAt(j);
                          ...
                          <tr class="<%=curSubClass_Row%>">
                            <td class='text'>
                              <input type="radio" name="CodeGestore" value="<%=myelement.getCODE_GESTORE()%>" onClick="setCodeHidden(this);">
                              <input type="hidden" name="tipo_sistema_mittente_<%=myelement.getCODE_GESTORE()%>" value="<%=myelement.getTIPO_SISTEMA_MITTENTE()%>">
                            </td>
                            
                            <td><%=myelement.getCODE_GESTORE()%></td>
                            <td><%=myelement.getDESC_GESTORE()%></td>
                            <td><%=myelement.getCODE_PARTITA_IVA()%>&nbsp;</td>
