...
<%
  ...
  String strCodeComponente =  Misc.nh(request.getParameter("CodeComponente"));
  ...
  if(!codice.equals("")&& intAggiornamento != 0){
    ...
    //AP Aggiunta modifica gestione descrizione 
    newComponente.setFLAG_MODIF(strFlagModificaDescrizione);
    ...
    {
      esito = componenti.insComponente(newComponente);
      if(esito.equals("")){
        messaggio = "Inserimento nuova Componente effettuata correttamente.";
      }else{
        messaggio = esito;
      }
    }
    ...
    {
      esito = componenti.aggiorna_componente(newComponente);
      
      if(esito.equals("")){
        messaggio = "Aggiornamento Componente effettuato correttamente.";
      }else{
        messaggio = esito;
      }
    }
    ...
    {
      esito = componenti.cancella_componente(strCodeComponente);
      
      if(esito.equals("")){
        messaggio = "Cancellazione Componente effettuata correttamente.";
      }else{
        messaggio = esito;
      }
    }
  ...
  {
    accountVector = componenti.getComponentiFiltro(strAccount,strDescAccount);
...
<!--AP Aggiunta modifica descrizione-->
  ...
  objForm.action = "pre_inserimento_componentiVis.jsp";
  objForm.submit();
}
...
function ONAGGIUNGI() {
  var URL = '';
  URL='pre_inserimento_componenti.jsp';
  openCentral(URL,'Componenti','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',900,500);
}

function ONAGGIORNA() {
  var URL = '';
  URL='pre_aggiornamento_componenti.jsp?code_componente='+document.frmDati.strCodeCompo.value;
...
function ONCANCELLA() {
  if(confirm("Attenzione!!! Si vuole procedere con la cancellazione della componente selezionata?")){
...
function ONMODIFICADESC() {
  var URL = '';
  URL='pre_aggiornamento_componenti.jsp?code_componente='+document.frmDati.strCodeCompo.value + '&modifica_descrizione=1';
  openCentral(URL,'Componenti','directories=no,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no',600,400);
 }
...
function setCodeHidden(obj){
  ...
  document.frmDati.strCodeCompo.value=obj.value;
}
</SCRIPT>
</HEAD>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
				<tr>
					<td>
					  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="#0A6B98">
						<tr>
						  <td bgcolor="#0A6B98" class="white" valign="top" width="91%">&nbsp;Componenti</td>
              ...
              <tr>
                <td>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="<%=StaticContext.bgColorTabellaGenerale%>">
                    ...
                    <tr>
                      <td width="20%" class="textB" align="right">Codice Componente:&nbsp;</td>
                      <%
                      if(strAccount.equals("%"))
                        strAccount = "";
                      %>
                      <td width="20%" class="text"><input class="text" type='text' name='txtAccount' value='<%=strAccount%>' size='15'></td>
                      <td width="20%" class="textB" align="right" nowrap>Descrizione Componente:&nbsp;</td>
                      <%
                      if(strDescAccount.equals("%"))
                        strDescAccount = "";
                      %>
                      ...
                      <td>
                        <table width="100%" align='center' border="0" cellspacing="0" cellpadding="0">
                          ...
                          <tr class="<%=curSubClass_Row%>">
                            <td class='text'>
                              <input type="radio" name="CodeComponente" value="<%=myelement.getCODE_COMPONENTE()%>" onClick="setCodeHidden(this);">
                              <input type="hidden" name="tipo_sistema_mittente_<%=myelement.getCODE_COMPONENTE()%>" value="<%=myelement.getTIPO_SISTEMA_MITTENTE()%>">
                            </td>
                            <td><%=myelement.getCODE_COMPONENTE()%></td>
                            <td><%=myelement.getDESC_COMPONENTE()%></td>
                          </tr>
                          <tr>
                            <td colspan='3' bgcolor="<%=StaticContext.bgColorCellaBianca%>"><img src="<%=StaticContext.PH_COMMON_IMAGES%>pixel.gif" width="1" height='2'></td>
                          </tr>
