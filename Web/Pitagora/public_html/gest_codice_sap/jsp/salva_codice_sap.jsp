...
<%=StaticMessages.getMessage(3006,"salva_codice_sap.jsp")%>
</logtag:logData>
<%
...
//    String controlloDataFine = data_fine_valid.substring(6,10)+ data_fine_valid.substring(3,5) + data_fine_valid.substring(0,2)   ;      
    ...
    ret = promo.checkGestoreSap(row);
     if(!ret)
     {
       response.sendRedirect("messaggio.jsp?messaggio=" + java.net.URLEncoder.encode("La data inizio validita' del nuovo codice gestore non puo' essere uguale o minore della data fine validita' del codice gestore cessato.",com.utl.StaticContext.ENCCharset));
       break;
    } 
       
   ret = promo.insertGestoreSap(row);
     if(ret)
     {
       response.sendRedirect("messaggio.jsp?messaggio=" + java.net.URLEncoder.encode("Codice Gestore Sap inserito con successo.",com.utl.StaticContext.ENCCharset));
    } else
    {
  ...
  if ( ret && operazione == 0 ) {
    response.sendRedirect("salva_codice_sap.jsp" + strQueryString);
...
<% } %>
<form name="formSalva" method="post" action="salva_codice_sap.jsp<%=strQueryString%>">
