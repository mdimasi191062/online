...
<%=StaticMessages.getMessage(3006,"InsAssOfPsXContrWfSp.jsp")%>
</logtag:logData>
<%
// act
...
if ((act!=null)&&(act.equalsIgnoreCase("conferma")))
  ...
  if ((msg==null)||(msg.equals("")))
    ...
    if (!data_ini_d.after(data_max))
      msg="La data inizio validita' deve essere maggiore della massima data inizio tra Contratto, P/S e Oggetto di Fatturazione";
    }
  ...
  %>
  <EJB:useHome id="homeOfPsXContrSTL" type="com.ejbSTL.AssOfPsXContrSTLHome" location="AssOfPsXContrSTL" />
  <%
  // controllo esistenza
  if ((msg==null)||(msg.equals("")))
    {
    //">>> Controllo esistenza!!!!"
    AssOfPsXContrSTL remoteOfPsXContrSTL=homeOfPsXContrSTL.create();
    int ret=0;
    
    if ( "999".equals(tipo_funz) || "998".equals(tipo_funz) ) {
        ret = remoteOfPsXContrSTL.check_esiste_clus(cod_tipo_contr,cod_contr,cod_of,cod_ps,cod_cluster,tipo_cluster, cod_tipo_contr);
    } else {
        ret = remoteOfPsXContrSTL.check_esiste(cod_tipo_contr,cod_contr,cod_of,cod_ps);
    }
    ...
    if (ret>0)
      ...
      if ( "999".equals(tipo_funz) || "998".equals(tipo_funz)) {
        aperte = remoteOfPsXContrSTL.check_aperte_clus(cod_tipo_contr,cod_contr,cod_of,cod_ps,cod_cluster,tipo_cluster, cod_tipo_contr);
        data_ini_min=remoteOfPsXContrSTL.getMinDataIniClus(cod_tipo_contr,cod_contr,cod_of,cod_ps,cod_cluster,tipo_cluster);
      } else {
        aperte = remoteOfPsXContrSTL.check_aperte(cod_tipo_contr,cod_contr,cod_of,cod_ps);
        data_ini_min=remoteOfPsXContrSTL.getMinDataIni(cod_tipo_contr,cod_contr,cod_of,cod_ps);
      }
      ...
      if (aperte>0)
        ...
        if ( "999".equals(tipo_funz) || "998".equals(tipo_funz)) {
            data_fine_max = remoteOfPsXContrSTL.getMaxDataFineCluster(cod_tipo_contr,cod_contr,cod_of,cod_ps,cod_cluster,tipo_cluster, cod_tipo_contr);
        } else {
            data_fine_max = remoteOfPsXContrSTL.getMaxDataFine(cod_tipo_contr,cod_contr,cod_of,cod_ps);
        }
        ...
        if ((data_fine==null)||(data_fine.equals("")))
          ...
          { // data-fine impostata
            ...
            Date data_ini_min_d=datafmt.parse(data_ini_min);
           if (!data_fine_d.before(data_ini_min_d))
               msg="La data fine deve essere minore del minore data inizio delle associazioni esistenti";
          }
        }
...
if ((act!=null)&&(act.equalsIgnoreCase("classe")))
  ...
  String tipo_funz=request.getParameter("tipo_funz");  
  %>
  <EJB:useHome id="homeAssOfPsContr" type="com.ejbSTL.AssOfPsXContrSTLHome" location="AssOfPsXContrSTL" />
  <%
  AssOfPsXContrSTL remote=homeAssOfPsContr.create();
  
  
  if ( "999".equals(tipo_funz) || "998".equals(tipo_funz) ) {
    canone_elem=remote.check_canone_exist_cluster(cod_tipo_contr,cod_contr,cod_cof,cod_ps,cod_cluster,tipo_cluster);
  } else {
    canone_elem=remote.check_canone_exist(cod_tipo_contr,cod_contr,cod_cof,cod_ps);
  }
  ...
  if (flg_ModAppl.equals("false"))
      {
         ...
         //30102002 session.setAttribute("flg_ModAppl","true");
...
if ((act!=null)&&(act.equalsIgnoreCase("contratto")))
  {
  String cod_tipo_contr=request.getParameter("cod_tipo_contr");
  String cod_contr=request.getParameter("cod_contr");
  String flag_sys=request.getParameter("flag_sys");
  ...
  %>
  <EJB:useHome id="homeContr" type="com.ejbSTL.ContrattoSTLHome" location="ContrattoSTL" />
  <%
  ContrattoSTL remoteContr= homeContr.create();
 //if (remoteContr.check_fatt_lu(cod_tipo_contr,cod_contr,flag_sys)!=0)
   //     msg="Già effettuata una fatturazione con listino unico per il contratto selezionato";
