...
<%=StaticMessages.getMessage(3007,"Stati di Elaborazione Batch Import")%>
<%=StaticMessages.getMessage(3006,"ver_upload.jsp")%>
</logtag:logData>
<%
...
// Lettura del Numero di record per pagina (default 50)
...
<%
   ...
   {
           StatiElabBatch = remoteStatiElabBatch.getStatiElabBatchImportFile(codeFunzione, codeStatoBatch, codeUtente, data_ini, data_fine);
...
<TITLE>
Stati Elaborazione Batch Import File
</TITLE>
...
function ONDETTAGLI()
{
    ...
    var VerificaScarti = "ver_dettaglioScarti.jsp?codeElab="+codeElab+"&flagSys="+flagSys;
    openDialog(VerificaScarti, 569, 400, '');
}
...
function downloadFile(file){
    //alert(file);
    nomeFile = file;
    document.frmSearch.nomeFileHiddenDownload.value=nomeFile;
    //alert(document.frmSearch.nomeFileHiddenDownload.value);
    document.frmSearch.action="../../estrazioni/jsp/download_file.jsp";
...
<form name="frmSearch" onsubmit="submitFrmSearch('0');return false" method="get" action="ver_upload.jsp">
...
<table align=center width="90%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><img src="../../stati_elab/images/titoloPagina.gif" alt="" border="0"></td>
  </tr>
  <tr>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
				<tr>
					<td>
					  <table width="100%" border="0" cellspacing="0" cellpadding="1" bgcolor="<%=StaticContext.bgColorHeader%>">
              <tr>
                <td bgcolor="<%=StaticContext.bgColorHeader%>" class="white" valign="top" width="91%">&nbsp;Stati Elaborazione Batch Import File</td>
                        ...
                        <tr>
                             ...
                             <td width="40%" class="text" align="Left">                             
                                  <%
                                    ...
                                    Vector Funz = remoteComboFunzioni.getFunzioniUpload();
                                    if ((Funz!=null)&&(Funz.size()!=0))
                                      ...
                                      // Visualizzo elementi
                                         ...
                                         for(Enumeration e = Funz.elements();e.hasMoreElements();)
                                         {
                                          ...
                                          //if(elem.getCodeFunzione().equals(StaticContext.RIBES_IMPORT_PER_PS_SAP_SP) || elem.getCodeFunzione().equals(StaticContext.RIBES_IMPORT_TARIFFE_SP) || elem.getCodeFunzione().equals(StaticContext.RIBES_IMPORT_PROMOZIONI) ||  elem.getCodeFunzione().equals(StaticContext.RIBES_IMPORT_FATTURE) )
                                          //{
                                            String selA="";
        ...
        <tr> 
          <td colspan='7' bgcolor="<%=StaticContext.bgColorCellaBianca%>"><img src="../../common/images/body/pixel.gif" width="1" height='2'></td>
            ...
            <tr> 
              ...
              <td bgcolor='<%=StaticContext.bgColorTestataTabella%>' class='textB'>File </td>
            </tr>
            ...
            <pg:param name="act" value="<%=act%>"></pg:param>
                ...
                %>
                 ...
                 <% } %> 
                 			...
                 			<% 
                	ScartiImportFile = remoteStatiElabBatch.getElencoScartiImportFile(obj.getCodeElab());
                	Object[] scarti=ScartiImportFile.toArray();                  
                  if ((ScartiImportFile!=null)&&(ScartiImportFile.size()!=0))
                 	{
                   	I5_6ELAB_UPL_FILES_ROW scarto=(I5_6ELAB_UPL_FILES_ROW)scarti[0];
                   	if ((scarto.getFILE_DOWNLOAD()==null) || (scarto.getFILE_DOWNLOAD().equals("")))
                    {%>
                    		<td bgcolor="<%=bgcolor%>" class='text' width="8%">&nbsp;</td>
                 	<% }else{ 
                        if(obj.getCodeFunz().compareTo(StaticContext.RIBES_IMPORT_FATTURE)==0)
                        {%>
                        <td bgcolor="<%=bgcolor%>" class='text' width="8%"><a href="#" onclick="downloadFile('<%=scarto.getFILE_DOWNLOAD()%>');" >ATTENZIONE! <br/>Fatt. Manuale</a> </td>
                      <% }else{ %>
                     		<td bgcolor="<%=bgcolor%>" class='text' width="8%"><a href="#" onclick="downloadFile('<%=scarto.getFILE_DOWNLOAD()%>');" >ATTENZIONE! <br/>Ripr. Tariffe</a> </td>
                 <%} 
                  } 
                 }
                 %>
