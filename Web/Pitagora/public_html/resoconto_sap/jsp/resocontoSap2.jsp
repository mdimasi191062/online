...
<%=StaticMessages.getMessage(3006,"resocontoSap2.jsp")%>
</logtag:logData>
...
<%
  ...
  String strParamJPUB2 = Misc.nh(request.getParameter("servizi_jpub2"));
  String strParamJPUBS = Misc.nh(request.getParameter("servizi_jpubS"));
  String strParamCICLO = Misc.nh(request.getParameter("ciclo"));
  ...
  strCodeFunz = "RESOCONTO_SAP";
  strUsrProfile = ((clsInfoUser)session.getAttribute(StaticContext.ATTRIBUTE_USER)).getUserName();  
  
  try{
    int risposta = 0;
    
    strResponse = String.valueOf(remoteCtr_ElabAttive.lancioBatchResocontoSAP(strCodeFunz,strUsrProfile,strParamJPUB2,strParamJPUBS,strParamCICLO));
