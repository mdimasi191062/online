...
function CaricaTabellaTariffe(){
	...
	DeleteAllRows(objTableBody);
		...
		objCell = objRow.insertCell();
                objCell.innerHTML = '<TD align= "center" >';
                objCell = objRow.insertCell();
		objCell.innerHTML = 'Importo Tariffa : ';
		objCell = objRow.insertCell();
		objCell.innerHTML = '<INPUT typeTxt="TARIFFA" class="textNumber" id="TAR-tcf" name="TAR-tcf"  Update="false" >';
		setObjProp(document.all('TAR-tcf'),'label=Tariffa' + propertyInput);
                objCell = objRow.insertCell();
                objCell.innerHTML = '</TD>';
      //     alert('CaricaTabellaTariffe');     
...
function CambiaTab(obj){
 ...
 function EliminaTariffa(Tariffa){
 	if(confirm('La tariffa verrà eliminata in modo permanente.\n\rContinuare?')){
		...
		frmDati.action = 'cbn1_controller_accordi.jsp?Operazione=Del';
		frmDati.submit();
	}
 }                
